...
| ワークツリー隔離 (`isolation: worktree`) | **実験的サポートあり（CLI 0.154.0〜）** | Codex 0.154.0 で新規／フォークセッション用の隔離チェックアウトを作れる実験的 worktree サポートが入った。ただし実験的扱いのため、当面の既定は従来どおり別ディレクトリで `codex exec --cd <worktree>` を実行する運用。検証後に採否を判断する |
