...
> **2026-09-03 更新（Codex 0.153.0）**: `Interrupt` が公式 hooks ドキュメントに収載され、Codex の対応イベントは 12 種になった。
...
| 概念 | Claude Code | Codex CLI | 抽象化の方針 |
|:--|:--|:--|:--|
| **フックシステム** | 包括的なイベント駆動フック（17+ イベント） | 12 イベント（0.150.0+） | 共通イベントを抽象化。差分は通知系・UI 系のみ |
...
| **Interrupt** | なし | サポート（0.150.0+） | Codex 固有。ターン中断の記録・後始末用。中断は阻止できず、既定タイムアウト 1 秒・上限 3 秒 |
