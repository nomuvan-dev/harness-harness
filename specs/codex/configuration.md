...
**実行とライフサイクル**（2026-09-01 に公式 hooks ドキュメントへ収載）:

- **既存の MCP 接続を使う**。フック側でサーバーを起動・再接続することはない
- ツールが blocking decision を返した場合のみ操作をブロックできる。**エラー・サーバー不在・ツール利用不可では操作をブロックしない**
- **同期実行**。ツール承認を要求せず、他のフックを誘発しない
- タイムアウトはフック側とサーバー側の**短い方**が適用される。MCP elicitation の応答待ち時間はタイムアウトに算入されない
- `SessionStart` フックは MCP サーバーの準備完了前に走ることがあるが、その場合もセッションをブロックしない
- **`SessionEnd` は `mcp_tool` 非対応**

`input` のプレースホルダは `${field.nested}` 形式でフックイベントのドット区切りフィールドを読む。値全体を占めるプレースホルダは JSON 型を保ち、文字列の一部に埋め込まれた場合はテキストとして描画される。オブジェクト・配列は再帰的に展開される。
...
Windows のセッションが**1つのバックグラウンド Codex サーバーを共有**できるようになり、デーモンのライフサイクル制御コマンドが追加された。Mac/Windows 両対応方針の下で、Windows 側の起動コストを下げられる。

### 8.3 起動時のワークスペース制御ヘルパー実行（0.154.0 で修正）

0.154.0 で、**信頼が確立される前にワークスペース側が用意したヘルパーを起動時に実行しない**よう修正された。信頼していないリポジトリを開いた時点でリポジトリ側のコードが走る余地を塞ぐセキュリティ修正であり、**信頼していないリポジトリを扱う場合は 0.154.0 以降を使う**。
