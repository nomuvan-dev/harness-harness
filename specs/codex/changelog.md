...
最終更新: 2026-09-27（**CLI 0.157.1 安定版**（2026-09-26）を反映。Chores のみのパッチリリースで、公式リリースノートにも実質的な変更記載なし（「Release highlights could not be determined」表記）。GitHub リリースタグは 0.159.0-alpha.5（2026-09-26）まで進行。前回: **CLI 0.157.0 安定版**（2026-09-25）を反映。目玉は **GPT-6 Sol / Luna の CLI 正式対応（Amazon Bedrock サポート・旧モデルからの移行プロンプト付き）**、**フルスクリーントランスクリプトのデフォルト有効化＋Shift クリックでのテキスト選択拡張**、**対象インタラクティブセッションでのバックグラウンドサーバー自動起動**（サーバー設定非互換時のリカバリ選択肢付き。0.156.0 の `/daemon` の既定化方向）、**`f` ショートカットで別アプリで開いている会話をフォーク**（ドラフト・キュー済みプロンプト保持）、**`/import` がリモートセッション・ローカルバックグラウンドサーバーセッションでも利用可能に**、端末レンダリング改善（Unicode 箇条書き・チェックボックス・数式整列）。修正はネットワーク制限のリダイレクト・継続 HTTP / WebSocket 通信への適用強化（ポリシー変更時のキャンセル含む）、プロキシルーティング、音声会話のスレッド切替時維持など。GitHub リリースタグは 0.158.0-alpha.15（2026-09-25）まで進行。前回: **CLI 0.156.0 安定版**（2026-09-22）と **CLI 0.156.1 ホットフィックス**（2026-09-23）を反映。目玉は **`/tui` フルスクリーン UI**、**音声会話のデフォルト有効化（F8 / `/voice settings`）**、**`/usage` アナリティクスダッシュボード**、**worktree サポートのデフォルト有効化＋agent command center からの worktree セッション作成**、**`/daemon` / `--no-daemon`**、**`friendly` / `pragmatic` personality 設定の非推奨明文化**。0.156.1 で GPT-6 Sol / Luna がモデルピッカーに追加された。※2026-09-23 巡回では「安定版 0.155.1 のまま」と判定していたが、rust-v0.156.0 / 0.156.1 は安定版としてリリース済みだった（タグ見落とし、または巡回後の安定版マーク）。前回: **CLI 0.155.1 安定版**（2026-09-18）を反映。バグ修正のみのパッチリリース：新規ローカル TUI セッションで reasoning サマリを既定無効（`none`）に戻し、reasoning サマリ非対応プロバイダでのリクエスト拒否を修正（#46467）。明示的な reasoning-summary 設定は維持される。GitHub リリースタグは 0.156.0-alpha.8（2026-09-19）まで進行。前回: **CLI 0.155.0 安定版**（2026-09-17）を反映。ハーネス観点の目玉は **agents overview のタスク管理＋managed worktree の確認付き削除**（0.154.0 の実験的 worktree サポートの管理面強化）、**MCP リクエストの Touch ID 検証**（Mac）、**デーモン更新スケジュール設定と再起動後のスレッド/goal 復旧**、**WSL サンドボックスエスケープのブロック**。SDK/ランタイム公開が安定版 CLI と同一バージョンで揃う運用に変更。GitHub リリースタグは 0.156.0-alpha.2（2026-09-18）まで進行。前回: **Python SDK 0.154.0**（2026-09-10）を反映: `max` / `ultra` の reasoning-effort 値、`ExternalMessage`（ツール級権限で外部コンテンツをターンに注入。ユーザー認可は付与しない）、resume/fork の `include_turns`・単発ターンの `turn_service_tier` が追加され、`HookMetadata` のアクセスが `.root` 経由に変わる破壊的変更あり。CLI 安定版は **0.154.0**（2026-09-09）のまま。GitHub リリースタグは 0.155.0-alpha.6（2026-09-15）まで進行中）

（2026-09-23 巡回: 安定版・SDK とも新リリースなし（※2026-09-24 訂正: rust-v0.156.0（2026-09-22）/ 0.156.1（2026-09-23）は安定版としてリリース済みだった。次エントリ参照）。安定版 CLI は 0.155.1 のまま、GitHub リリースタグは 0.157.0-alpha.9（2026-09-22）までプレリリースのみ進行。**アナウンス（2026-09-22）: 新モデル GPT-6 Sol / GPT-6 Luna 発表**。GPT-5.6 世代の後継で、**Sol は複雑なコーディング・エージェントワークフロー向け、Luna は焦点を絞った大量処理タスク向け**。トークン単価は GPT-5.6 世代より低い。Plus / Pro / Business / Enterprise / Edu にロールアウト中（Free / Go はデスクトップアプリで Luna のみ。Enterprise は管理者による有効化が必要。ChatGPT では Work と Codex で利用可、Chat は不可）。CLI では `/model` または `codex --model gpt-6-sol` / `codex --model gpt-6-luna` で選択）

（2026-09-22 巡回: 安定版・SDK とも新リリースなし。安定版 CLI は 0.155.1 のまま、GitHub リリースタグは 0.156.0-alpha.16（2026-09-21）までプレリリースのみ進行。公式 changelog の最新エントリも 2026-09-18（0.155.1 / ChatGPT for iOS 1.2026.251）から変わらず）

（2026-09-21 巡回: 安定版・SDK とも新リリースなし。安定版 CLI は 0.155.1 のまま、GitHub リリースタグは 0.156.0-alpha.9（2026-09-20）までプレリリースのみ進行。**アナウンス: GPT-5.3-Codex-Spark（research preview）が 2026-09-14 付で廃止**。ChatGPT デスクトップアプリ / Codex CLI / IDE 拡張で利用不可となり、推奨モデルのいずれかへ切り替えること。`gpt-5.3-codex-spark` を参照する保存済み設定・カスタムエージェント・スクリプトも要更新）

（2026-09-18 巡回: 安定版・SDK とも新リリースなし。安定版 CLI は 0.154.0 のまま、GitHub リリースタグは 0.155.0-alpha.16（2026-09-17）までプレリリースのみ進行。公式 changelog の最新エントリも GPT-5.5 退役アナウンス（2026-09-14）のまま変わらず）

（2026-09-17 巡回: 安定版・SDK とも新リリースなし。0.155.0-alpha.11（2026-09-16）までプレリリースのみ進行。**重要アナウンス（2026-09-14）: GPT-5.5 が 2026-10-14 に ChatGPT / ChatGPT Work / Codex から退役**。ChatGPT サインインの Codex では退役日までに `gpt-5.5` → `gpt-5.6-sol`（GPT-5.6 Sol）へ切り替えること。ワークスペース既定・保存済みモデル設定・managed 設定・カスタムエージェント・スケジュールタスク・スクリプトの `gpt-5.5` 指定も要更新。OpenAI API には適用されない）

（2026-09-16 巡回: 安定版・SDK とも新リリースなし。0.155.0-alpha.4〜6 のプレリリースのみ進行）

---

## CLI 0.157.1 (2026-09-26)

**Chores のみのパッチリリース**

- 公式リリースノート・changelog とも実質的な変更記載なし（「Release highlights could not be determined: the supplied PR index is empty, and the GitHub tag comparison returned 404」表記のみ）

## CLI 0.157.0 (2026-09-25)

**GPT-6 Sol / Luna 対応・フルスクリーントランスクリプト既定化・バックグラウンドサーバー自動起動**

### 新機能

- **GPT-6 Sol / Luna 追加**: Amazon Bedrock サポートと旧モデルからの移行プロンプト付き
- **フルスクリーントランスクリプトをデフォルト有効化**、Shift クリックでテキスト選択を拡張
- **バックグラウンドサーバーの自動起動**: 対象のインタラクティブセッションで自動起動。サーバー設定が非互換の場合はリカバリ選択肢を提示
- **`f` ショートカット**: 別アプリで開いている会話をフォーク（ドラフト・キュー済みプロンプトを保持）
- **`/import` の利用範囲拡大**: リモートセッションとローカルバックグラウンドサーバーセッションでも利用可能に
- 端末レンダリング改善: Unicode 箇条書き・チェックボックス・数式の整列・最適化記法

### 修正

- ネットワーク制限をリダイレクト・継続中の HTTP / WebSocket 通信にも強制（ポリシー変更でアクセスが取り消された際のキャンセル含む）
- 設定済みプロキシのルーティング修正（realtime 接続・スタンドアロン Web 検索・検索リダイレクト）
- スレッド切替時にアクティブな音声会話を維持
- ファイルアップロードの一時失敗リトライ追加・タイムアウトを5分に延長
- tmux マウス設定の尊重、SSH 越し Terminal.app のネイティブスクロールバック復元

---

## CLI 0.156.1 (2026-09-23)

`npm install -g @openai/codex@0.156.1`。0.156.0 のホットフィックス。

- **GPT-6 Sol / GPT-6 Luna をモデルカタログに追加し、モデルピッカーから選択可能に**（#47405）。レート制限切替プロンプトは GPT-6 Luna を推奨するようになった（2026-09-22 発表の新モデルへの CLI 側対応）

## CLI 0.156.0 (2026-09-22)

`npm install -g @openai/codex@0.156.0`。ハーネス観点で影響の大きい変更を抜粋。

**新機能**

- **`/tui` によるオプションのフルスクリーン UI**: 次回起動から適用。トランスクリプト検索・マウス選択・右クリックコピーに対応（#46732, #46734, #46883, #46895）
- **音声会話がデフォルト有効化**: F8 トグル、`/voice settings` での音声選択、Linux / Windows 向け音声ランタイム同梱（#44921, #46071, #44622, #44714, #44922）。0.155.0 の実験的 `/voice` が既定機能に昇格
- **`/usage` がアナリティクスダッシュボードに拡張**: アカウント使用量・トークン合計に加え、プラグイン / スキルのアクティビティを閲覧可能（#45764, #45769）
- **worktree サポートがデフォルト有効化**（#44870）: agent command center でステータスによるタスクフィルタと worktree セッション作成が可能に（#46839, #45276）。0.154.0 の実験的サポートが既定化されたため、**「外側で `git worktree add` → `codex exec --cd`」を既定とする従来方針は要再検証**
- **新テーマ 6 種＋Mermaid 図・数式（display equations）のインラインレンダリング**（#46504, #46054, #45612）
- **`/daemon` でローカルバックグラウンドサーバーを更新、`--no-daemon` でデーモンをバイパス**（#45854, #46088）

**修正**

- ターンの失敗・中断・サブエージェント完了イベント受信時に、ストリーム済みの回答とプランを保持（#45549, #46867）
- tmux / SSH セッションのクリップボード転送を修正し、端末が貼り付けを個別キーストロークで送る場合もタブインデントを保持（#45457, #45454）
- セッション再開時に Plan mode を復元し、過去プロンプト編集時にスレッド ID と設定を保持（#45519, #45845）
- **sandbox 隔離ギャップを修正**（セキュリティ、#44639, #45984, #46500）: Windows へのインバウンド接続、Linux / macOS の特権ソケット、macOS の読み取り専用ファイルハンドル経由の書き込みを遮断

**非推奨・ドキュメント**

- **非推奨の `friendly` / `pragmatic` personality 設定が応答スタイルを選択しなくなった**ことを明文化（#45809）。TUI からの personality 選択も削除された（#44935, #44946）
- ネットワークプロキシの allow / deny パターンで `?` は 1 文字にマッチすることを明記（#46027）

---

## CLI 0.155.1 (2026-09-18)

`npm install -g @openai/codex@0.155.1`。バグ修正のみ。

- **新規ローカル TUI セッションの reasoning サマリを既定無効（`none`）に戻した**（#46467）。reasoning サマリ非対応プロバイダ（サードパーティ / セルフホスト等）でのリクエスト拒否リグレッションを修正。明示的な reasoning-summary 設定は従来どおり有効

## CLI 0.155.0 (2026-09-17)

`npm install -g @openai/codex@0.155.0`。ハーネス観点で影響の大きい変更を抜粋。

**新機能**

- **実験的 `/voice` 会話**: 対応ビルドでライブ文字起こしとマイクコントロール付きの音声会話。`/experimental` で有効化（#43581, #43651, #44331）
- **TUI の可視性向上**: ステータス行にライブ reasoning サマリ、成功ターン後に完了タイムスタンプを表示（#43558, #43921）
- **agents overview の管理機能**: タスクの非表示・アーカイブ・削除、worktree の所有者情報表示、クリーンな managed worktree の確認付き削除（#43942, #44424, #44433）。0.154.0 で入った実験的 worktree サポートの管理面が充実
- **ローカル TUI セッションの MCP リクエストに Touch ID 検証**（対応 Mac、#43624, #43712, #43715）
- **デーモン更新スケジュールの設定と `codex app-server daemon update`**: デーモン再起動後も保存済みスレッドとアクティブな goal を復旧可能（#43542, #43562, #44314）
- **Amazon Bedrock の AWS 認証情報をコマンドから取得**: キャッシュ・期限ベースの更新・認証リカバリ付き（#44028）

**修正**

- コンパクションがターン開始前に失敗しても、受理済みプロンプトが保存されるようになった（#44487）
- MCP サーバーが期限切れ OAuth 資格情報を正確に報告し、トークン更新失敗時に再接続ガイダンスを表示（#43947, #44359）
- アカウント切り替え時に、前のアカウントの remote-control セッション・WebSocket キャッシュ・モデルカタログを無効化（セキュリティ、#43906, #44341, #44489）
- **制限付き WSL サンドボックスからの Windows プロセスエスケープをブロック**し、brokered shell スナップショットの資格情報露出を強化（セキュリティ、#44286, #43909, #44040）
- 自動承認レビューの信頼性向上: アクションと認可証跡の完全保持、transient 失敗のリトライ、レビュー失敗と unsafe-action 判定の区別（#44482, #44569, #44570）

**その他**: Python SDK とランタイムの公開を安定版 CLI リリースと同一バージョンで揃える運用に変更（#44067）→ SDK 0.155.0 も追って公開される見込み。
...
## CLI 0.154.0 (2026-09-09)

ハーネス観点で影響の大きい変更を抜粋。

**新機能**

- **実験的な worktree サポート**: 新規セッションやフォークしたセッション用に隔離チェックアウトを作成できるようになった。harness-harness の worktree 運用（docs/conventions.md）と直接噛み合う機能で、これまで外側から `git worktree add` → `codex exec --cd <worktree>` としていた手順を Codex 側で完結できる可能性がある。**実験的扱いなので当面は外側運用を既定とし、本機能は検証したうえで採否を判断する**
- **作業継続中のインライン回答**: Codex が作業を続けたまま質問に答えられるようになった。提示された選択肢からの選択と自由入力の両方に対応
- **Windows のバックグラウンド Codex サーバー共有**: Windows セッションが1つのバックグラウンド Codex サーバーを共有できるようになり、デーモンのライフサイクル制御コマンドが追加された。Windows 側のハーネス（Mac/Windows 両対応方針）で起動コストを下げられる
- **GPT-6-Astra がモデルピッカーと Amazon Bedrock カタログで利用可能**（0.153.x からの継続的な露出拡大）
- Vim 編集に `R` 置換モードが追加され、undo とドットリピートに対応。Escape の扱いも安定化
- レスポンスのコピーがリッチテキストアプリで書式を保持するようになり、`/copy` でステータス出力もコピーできるようになった

**修正**

- **起動時に信頼が確立される前のワークスペース制御ヘルパーを実行しないよう修正**（セキュリティ）。信頼していないリポジトリを開いた時点で、リポジトリ側が用意したヘルパーが走る余地を塞ぐもの
- 既存セッションが、外部でプラグインを更新した後に新規インストールされたプラグインツールを取り込み、スキルを再読み込みするようになった
- MCP 接続が OAuth トークン更新を協調させ、更新失敗時にログインチャレンジを表示するようになった
- リモートの resume / fork が保存済み権限を維持するよう修正。新規セッションはサーバー側のモデル既定に従う
- 別アプリで開いているセッションを resume した場合、読み取り専用トランスクリプトとリトライ操作が表示されるようになった
- 自動承認レビューがコンパクション越しに認可コンテキストを保持し、無効化された承認を拒否するようになった

---

## CLI 0.153.4 (2026-09-04)

- **GPT-6-Astra がバンドルのモデルピッカーに表示されるようになり、モデルを明示設定していない場合の既定モデルになった**（#42874）。0.153.1〜0.153.3 の「既定は変えずカタログのみ追加」という位置づけからの方針転換。**ハーネス側で `model` を明示していないプロジェクトは、更新するだけで使用モデルが変わる**点に注意
- 修正: Astra の非同期質問ガイダンスを、当該ツールがセッションで使える場合のみ適用するよう限定（#42878）

---

## CLI 0.153.3 (2026-09-04)

- **GPT-6-Astra を Amazon Bedrock のモデルカタログに追加**（#42805）。Mantle / Runtime の global・US ルート向け
- 修正: GPT-6-Astra の非同期の確認質問ガイダンスを、サポートされているツールを使う形に訂正し、テキストのみ受理する点を反映（#42809）
...
## CLI 0.152.0 (2026-09-01)

- **プランニングツール（`update_plan`）が既定で無効化**（#41744）。使うには `tools.update_plan.enabled = true` を明示設定する。**プラン提示を前提にしたハーネス（AGENTS.md の運用ルール等）は要見直し**
- **MCP ツール単位の `output_token_limit` 設定**（#41421）。ツールごとに出力トークン上限を設定でき、セッション再開をまたいでも truncation が一貫する。**設定キーの配置は 2026-09-02 時点で公式 config reference に未収載**
- **MCP サーバー名に `:` `@` `/` `.` を使用可能に**（#41700）。`@scope/pkg` のようなパッケージ形式の名前を CLI コマンド・認証まわり全体で扱える
- **app-server クライアントが `thread/shellCommand` のタイムアウトを設定可能に**（#41384）。1時間超の期限も指定できる
- **Vim モードの検索対応**（#41586）。ドラフト内で `/` `?` 検索、マッチのハイライト、`n` / `N` での反復移動
- **レート制限バナーからのアクション**（#41742）。使用量確認・クレジット管理・制限リセット・プラン管理へ導線
- **認証情報リフレッシュの進捗表示**（#41239）。TUI と `codex exec` の双方。Amazon Bedrock の再認証を含む
- プラグイン推薦の読み込みを起動時に開始し、初回ターンまでの待ちを短縮（#41375）
- 修正: 自動承認レビュー（Guardian）が長いメッセージとより大きなトランスクリプトを保持できるようになり、履歴コンパクション後もユーザー指示・回答・有効な承認を維持（#41931, #41660, #41846, #41852）
- 修正: 再開スレッドが作業ディレクトリ未指定時に保存済みのものを復元し、クライアントメタデータ更新でファイルシステム権限が失われない（#41567, #41464）
- 修正: MCP ツールがキャッシュ更新・リモートプラグイン変更をまたいで利用可能なままになり、認証リトライがヘルパー提供の最新ヘッダを使う（#41336, #41344, #41396, #41400）
- 修正: Windows の Microsoft Store 版 PowerShell でのサンドボックス実行、端末クエリでのサブプロセスハング、旧 JediTerm でのカーソル起因の表示崩れ（#41227, #41436, #41673）
- セキュリティ修正: クラウドタスク要求が信頼できないバックエンド URL を拒否し、リダイレクトを無効化して保存済み認証情報を保護（#41403）
- 修正: Vim 有効なコンポーザーが送信後・スラッシュコマンド実行後も Insert モードで新規ドラフトを開始（#41921）
- 修正: モデルピッカーを開いたときに選択中の項目を失わずに利用可能モデルを再取得（#41467）

---

## CLI 0.151.0 (2026-08-29)

- **オプション MCP サーバーのツール検出猶予期間（grace period）を設定可能に**（#41199）。従来は固定だった「必須でない MCP サーバーの起動待ち」を調整できる。**設定キー名は 2026-08-30 時点で公式 config reference に未収載**（`mcp_servers.<id>.startup_timeout_sec` / `startup_timeout_ms` / `required` は既存キー）
- **拡張（extensions）が MCP ツールの結果をモデルに届く前に検査・置換できるようになった**（#41202）。Claude Code の `PostToolUse` フックに相当する介入点が MCP ツール結果に対して開いた
- **プラグインカタログがリポジトリ単位の設定を合成**するようになり、不正な project marketplace があっても有効なプラグインを隠さずに報告する（#41208）
- 修正: TUI のターンをまたいで復元済み権限プロファイルを保持し、`/cd` がサンドボックス制限を弱めないようにした（#41192）
- 修正: モデル切替時・別モデルへのフォールバック時にツールの利用可否と reasoning effort が正しく保たれるようにした（#41195, #41206。Ultra reasoning のフォールバックがモデル依存に）
- 修正: リモートサンドボックスの強制を、executor の実際のホームディレクトリ・OS・パス規約に基づいて行うようにした（#41196, #41204, #41207, #41209。deny-read の照合も executor のパス意味論に合わせた）
- 修正: app-server 応答で MCP のツール／リソースの構造化エラーを保持（#41196）
- 修正: ネストしたサブエージェントのトークン使用量をルートゴールの予算に算入（#41183）
- 修正: 権限状態が変わった後に古い Guardian 分類結果がアクションを承認してしまう問題（#41196）

---

## CLI 0.150.1 (2026-08-27)

- 修正: リモートコンパクションが保持画像をトークン予算に算入するようになり、必要に応じて古い画像から切り落とす（既定挙動）

---

## CLI 0.150.0 (2026-08-26)

- **`Interrupt` フック（新イベント）**: アクティブなトップレベルターンが中断されたときに command / MCP ハンドラを実行できる。0.148.0 時点の 11 イベントに続く 12 番目（公式 hooks ドキュメントへの記載は 2026-08-28 時点で未反映。リリースノートのみ）
- **タスクの `@` メンション**: 他の Codex タスクを `@` で参照でき、ターミナルからエージェントにタスクの読み取り・作成・メッセージ送信を依頼できる
- **`/copy` のピッカー**: 応答全体・個別コードブロック・引用ブロックを選んでコピー
- **タスクの自動命名**: 無名のターミナルタスクに説明的なタイトルが自動付与され、`/rename` は会話内容から編集可能なタイトルを提案する
- **Markdown リンクのクリック可能表示**: 対応端末ではラベル表示。非対応端末では URL がそのまま残る
- **権限モード切替のキーバインド**: 権限モードをサイクルするショートカットをバインド可能に（`tui.keymap.<context>.<action>`）。Vim モードで `.` による直前編集の繰り返しに対応
- 修正: 未信頼プロジェクトではプロジェクトレベルの `AGENTS.md` 指示を供給しなくなった。権限変更後も managed deny-read ルールが維持される
- 修正: app-server 診断における認証情報の redact 強化（provider / 認証リフレッシュ / attestation フィールド）
- 修正: リモート MCP の bearer トークン解決、必須サーバー起動（旧 executor 互換を維持）
- 修正: Windows の昇格サンドボックス設定と Unicode ユーザーパス配下の起動エイリアス
- 修正: 端末を掴んだままのデタッチプロセスや端末出力バッファ満杯による Unix シャットダウンハング
- 修正: Amazon Bedrock モデルにおける会話コンパクションとマルチエージェント互換性
...
## CLI 0.149.0 (2026-08-20)

- **`codex agents` ダッシュボード**: タスクの検索・開始・オープン・リネーム・停止を行う対話的ダッシュボード。TUI の agents overview としても利用でき、ショートカットは設定可能
- **`codex queue`**: 既存のローカル / リモートセッションへメッセージを送り込むサブコマンド。アイドルセッションを確実に起こす。名前指定が重複した場合は最新セッションを優先
- **作業ディレクトリ操作コマンド**: TUI セッションで `/cd`・`/pwd`・`/cwd` が使えるようになった
- **Vim 編集の拡張**: 文字置換と `cw` / `c$` / `cc` などの change モーションに対応
- **`codex doctor` の診断拡張**: エンドポイント保護、ネットワーク / プロキシ障害、デスクトップアプリの状態、アップデート接続性を診断（Windows サンドボックス診断も追加）
- **SDK**: CLI config オーバーライドをそのまま渡せるようになり、推論努力度に `max` / `ultra` を選択可能に
- スキルカタログのトークンバジェットを設定可能化
- 修正: resume / fork したスレッドが、現在の既定へ暗黙にフォールバックせず元のアクティブな権限プロファイルを復元するようになった
- 修正: サブエージェント活動の二重表示と、サブエージェント通知 / 承認の TUI ルーティング
- 修正: Windows Terminal のスクロールバックにインライン TUI 履歴が残るようになった
- ドキュメント: secure devcontainer における DNS exfiltration リスクと信頼境界の限界を明記

---

## CLI 0.148.0 (2026-08-18)

- **`/export` によるトランスクリプト書き出し**: TUI の会話全体を Markdown としてクリップボードまたは新規ファイルへエクスポート
- **`codex exec fork`**: 非対話モードでセッションを分岐。TUI の resume ピッカーからはセッションのアーカイブ / 復元が可能に
- **起動中のプロンプト下書き**: TUI 初期化中にプロンプトを入力でき、起動中も resume / fork の進捗が表示される
- **`/status` にスレッド単位のクレジット / コスト表示**: 対象となる Business / Enterprise ワークスペースで、推定クレジット・USD コスト・モデル/推論/速度/トークンの内訳を `/status`・ステータスライン・ターミナルタイトルに表示
- **Amazon Bedrock Runtime を組み込みプロバイダ化**: `amazon-bedrock-runtime` プロバイダを追加。リージョン別 `bedrock-runtime` の OpenAI 互換エンドポイントに対し、プロバイダ単位の AWS プロファイル / リージョン / トランスポート上書きとベアラートークン認証（SigV4 併用）をサポート。GPT-5.6 は global 版と US クロスリージョン版を提供し、フォールバック/バックグラウンドタスクは global ルーティングを優先。当プロバイダでは Web 検索は無効
- **Hooks の非同期実行**: command ハンドラに `async = true` を指定するとバックグラウンド実行される（セッション単位の同時実行上限あり）。非同期フックは起動元の操作をブロック・停止・書き換えできず、結果は安全なターン境界（サンプリング後の現ターン、またはアイドル時は次のユーザープロンプト直前）で注入される。`SessionEnd` は同期のまま
- **Hooks から MCP ツールを呼び出し可能に**: ハンドラ種別 `mcp_tool`（`server` / `tool` / `input` / `timeout` / `statusMessage`）を実行対応。`input` 内のフック変数プレースホルダは JSON 型を保ったまま展開される。`SessionEnd` と MCP 呼び出し非対応ランタイムでは起動時警告を出してスキップ
- **skill-creator の整理**: 同梱 skill-creator ガイドをより絞った内容に変更し、未記入の TODO プレースホルダを検証で拒否するように
- **修正**: モデル切り替え / 設定更新が古い指示を残したり実行中ターンを途中で変更する問題、セッション再開時に保存済み作業ディレクトリと承認ポリシーが復元されない問題、プロバイダの一時障害からのターン再接続、OAuth 再認証後に Codex を再起動せず MCP サーバーが復帰、TUI 起動時にバッファ済み端末入力がプロンプトを誤発火する問題、CRLF 貼り付け / 折り返し空白 / 長い URL の描画、Linux・Windows でサンドボックス制限が拒否パス・読み取り不能パスに対して fail closed になるよう修正

## CLI 0.147.0 (2026-08-07)

- **ポータブル Agent Plugins**: プラグインをポータブル形式でインストールし、local / personal / workspace / remote のプラグインカタログを横断検索
- **会話セクション**: 会話を永続的・手動並び替え可能なセクションに整理し、長いトランスクリプトを段階的に閲覧
- **`--approve-for-me` フラグ**: 自動レビュー済み承認を有効化する新 CLI フラグ
- **Cursor スキルインポート**: Cursor 管理スキルをインポートし、インポート済み Claude / Cursor 会話の変更を重複作成せず同期
- **MCP 2026-07-28 プロトコル対応（オプトイン）**: ページネーション付きディスカバリ、マルチラウンドリクエスト、非ブロッキングサーバー起動をサポート
- **Bedrock 拡張**: キャッシュ済み Web 検索とリモート会話コンパクションを Amazon Bedrock で有効化
- **`codex exec --full-auto` 削除**: 代わりに `--sandbox workspace-write` を使用
- **修正**: 表示コマンド・再生履歴からのシークレット/ベアラートークンのマスキング、フォーカス復帰時のターミナル入力欠落防止、日本語・絵文字・ハイパーリンクの描画/カーソル位置補正、Windows バックグラウンドプロセスの割り込み・パス処理、未知ローカルプロジェクトへの明示的信頼要求
- **依存更新**: MCP SDK 3.0.0 / Ratatui 0.30.2 / V8 150.4.0
- 冗長な Linux バンドルアーカイブの発行を停止（標準 `codex-package-<target>` アーカイブを使用）
...
- **セッション命名・ピン留め**: `/new` / `/clear` で新セッションに名前を付けられるように。重要スレッドのピン留め、サイド会話を閉じずに切り替え可能に
- **Agent Plugins マニフェスト対応**: workspace プラグイン公開、Amazon Bedrock / Claude Code 向けの追加プラグインマーケットプレイスをサポート
- **スレッドフォーク強化**: ページネーション付き履歴でのフォークに対応。スレッド一覧に現れない一時フォークも可能に
- **リモート Code Mode**: app-server から WebSocket 経由でリモート Code Mode ホストに接続可能
- **スタンドアロン Web 検索**: 対応するカスタムモデルプロバイダで単体 Web 検索を有効化
- **executor 提供スキルの発見**: 明示選択されたスキルを含め、executor が提供するスキルと関連リソースを安全に読み取り
- 信頼性改善: プロキシ設定を全経路で尊重（認証・プラグインDL・MCP 認可・リモート実行・WebSocket・LM Studio）、MCP サーバーの自動再接続、タイトなコンテキスト予算下でのスキルカタログ保持改善と切り詰め警告

## CLI 0.145.0 (2026-07-21)

機能追加の大型リリース。

- **`/import` の大幅拡張**: Claude Code に加えて **Cursor からの移行**に対応。設定・MCP サーバー・プラグイン・セッション・コマンド・プロジェクトスコープのメモリまで選択的にインポート可能に
- **ページネーション付きスレッド履歴（実験的）**: 効率的な resume、検索、名前の永続化、サブエージェント対応、メモリ連携
- **マルチエージェント V2 の安定化（opt-in）**: サブエージェントごとのモデル・reasoning レベル・並列度の設定、ロール復元、エージェント間ナビゲーション改善
- **Amazon Bedrock ログイン（実験的）**: カスタムエンドポイント・認証に対応。Bedrock のデフォルトモデルは GPT-5.6 Sol
- **音声入出力**: 一般的なローカル音声フォーマットの入力・ツール出力に対応。ストリーミングのリアルタイム V3 会話を導入
- **ターミナル UI にインライン可視化リンク**: セキュアなクリック可能リンクで可視化を表示
- **プロンプト編集で文脈保持ブランチ**: 過去プロンプトの編集・リトライ時に元の会話・添付・メンションを保持したブランチを作成
- **安全性強化**: 強制 `rm` 検出の改善、full-access 確認の一貫化、ツール間での拒否理由の保持
- そのほか: 長い会話のターミナル応答性改善（インクリメンタル Markdown レンダリング）、MCP 起動タイムアウト強制・OAuth リフレッシュ直列化、Windows のネイティブ exec-server サンドボックス化、同梱 ripgrep を 15.2.0 に更新、バンドルの GPT-5.4 利用を GPT-5.6 Terra / Luna に移行
...
## CLI 0.144.5 (2026-07-16)

- **dangerous-command 検出を改善**: 強制 `rm` の変種をより広く検出し、コマンド拒否時により明確な理由を提示するように
