...
| claude.ai アカウント | claude.ai アカウントで有効なスキル | Cowork / クラウドセッションに加え、**そのアカウントでサインインしたターミナルセッションにも自動同期**（2026-09-17 ドキュメント改訂。同期の詳細は後述） |
...
| 上記いずれか + claude.ai 同期スキル | 素の `/<name>` は同期スキル以外が優先。**同期スキルは常に `/anthropic-skills:<name>` の完全名で呼び出せる**（v2.1.269 以降。それ以前は短縮名しか持たず、衝突時は完全にスキップされていた） |
...
ネストスキルは**起動時にはロードされない**ため、ロード前は補完に出ず名前指定で呼び出すこともできない。**ファイル操作を待たずに先読みさせたい場合は `/add-dir <サブディレクトリのパス>` を実行する**（v2.1.257 以降）。
...
> **注入コマンドの権限チェック（2026-09-17 ドキュメント改訂で明文化）**: 注入コマンドはレンダリング中に権限プロンプトを出さず、先に権限ルールと突合される。deny ルールにマッチすると `Shell command permission check failed for pattern "..."` で起動が中断。**auto モード以外**では、allow 以外の判定（ask ルール含む）でも同じエラーで中断するため、`allowed-tools` で事前承認しておく（deny / ask ルールは `allowed-tools` に関わらず中断）。**auto モードでは中断せず**、スキルは「まずこのコマンドを実行せよ」という指示付きでロードされ、Claude 自身の呼び出しが auto モードの通常チェックを通る（`agent` を設定したフォークスキルでは従来どおり中断）。
...
| `/diff` | ワーキングツリーの変更（Claude の編集を含む）をレビューする。**v2.1.260 以降**、フルスクリーンレンダリング時は会話の横に **diff パネル**を開き、Claude の編集に追随して更新され続ける（`/diff` 再実行かヘッダの `✕` で閉じる）。要件はフルスクリーン・git リポジトリ・端末幅 110 桁以上で、満たさない場合は従来の diff ビューア（プロンプト位置に開き Esc で閉じる）にフォールバックする。端末幅 144 桁以上なら Claude がファイル編集を始めた時点で自動的に開く。パネルでは行をマウス選択して次のプロンプトに添付でき、`Ctrl+X B`（`app:cycleDiffBase`）で比較基準を「今セッションの変更 → 未コミット全体 → 既定ブランチからの分岐点以降」と巡回する（プロジェクトごとに記憶）。テスト・生成ファイルと当セッション以前の変更は折りたたまれる |
| `/plan` | プランモード開始 |
...
**一時的エラー後の自動リトライ / 一時停止（v2.1.269、対話セッションのみ）**: 上記4種以外のエラーでターンが失敗した場合、原因を1行表示した上で挙動が分岐する:

- **リトライ**: サーバー過負荷・接続断など自然解消しがちな失敗は `Goal still active` で始まる通知に次回試行までの待ち時間を表示して自動リトライ。**3回の自動リトライ後は一時停止に移行**
- **一時停止**: API レート制限・claude.ai 利用上限・ターンを終了させた hook など、リトライしても繰り返すだけの失敗は `Goal paused` で始まる通知に原因を表示して待機。利用上限リセット待ち（`autoContinueAtUsageLimit`）中ならリセット時にゴール作業を再開

いずれの状態でもユーザーがメッセージを送れば即座に次のターンが開始される。`CLAUDE_CODE_GOAL_CHECKIN_MINUTES=0` はチェックインに加えて**自動リトライも無効化**する。
...
| `--append-subagent-system-prompt` | **サブエージェント**のシステムプロンプトへの追加指示（`-p` の非対話モードのみ。v2.1.205 以降）。内部的に `CLAUDE_CODE_APPEND_SUBAGENT_SYSTEM_PROMPT` を設定する |
| `--append-subagent-system-prompt-file` | 上記の追加テキストを**ファイルから読む**（コマンドラインに載せられない長文用。v2.1.261 以降）。`--append-subagent-system-prompt` との併用は不可。`-p` の非対話モードのみ有効 |
...
- **診断（v2.1.252 以降）**: `/skill-doctor` が**スキル単位のコンテキストコストと呼び出し回数**を出し、一度も使われていないスキルを名指しする。「同梱したが実際には呼ばれていないスキル」を剪定する際の一次情報
...
| 使われていないスキルを外す | `/skill-doctor` で未使用スキルを特定し無効化 | 判断材料が実測ベースになる。ただし v2.1.252 以降＋feature flag 取得が必要 |
