...
| `outputStyle` | 出力スタイル設定。組み込みは Default / Proactive / Explanatory / Learning に加え **Concise**（前置き・ナレーションを省き結果から書き出す。作業の徹底度は変えない。v2.1.237 追加）。`/config` の Output style 行から選択すると `.claude/settings.local.json` に保存される。**v2.1.251 以降、セッション中に切り替えると次のメッセージから新しいスタイルが適用される**（スタイルはシステムプロンプトの一部なので、その 1 回だけプロンプトキャッシュが再構築される）。v2.1.251 より前は `/clear` かセッション再起動が必要だった。なお**ターミナルではスタイルファイルを起動時に読む**ため、セッション中にファイルを新規作成・編集した場合は再起動が必要 |
...
| `skillOverrides` | スキル単位の表示制御。`off`（モデルと `/` から非表示）/ `user-invocable-only`（モデルから非表示）/ `name-only`（モデルには名前のみ提示し説明を出さない）。列挙されないスキルは `"on"` 扱い。v2.1.129 で実装が修正され機能するように。プラグインスキルには適用されない（`/plugin` で管理する）。**エイリアスをキーにした場合の扱い（v2.1.260 以降）**: managed 設定または `--settings` で渡したファイルでは、バンドルスキルのエイリアス（`/doctor` に対する `checkup` 等）をキーにしたエントリが**その背後のスキルに適用される**。ただしエイリアス経由では制限を強める方向にしか働かず、より見えるようにはできない。managed 設定でスキル自身の名前のエントリも設定されている場合はそちらが優先。**user / project / local 設定ではスキル名にのみマッチ**し、`review` のエントリはバンドルの `/code-review` にはエイリアス経由で適用されない |
