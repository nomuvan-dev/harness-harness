...
Claude Code は設定ファイルを監視し、変更を検知するとセッションを再起動せずに反映する（`permissions`・`hooks`・`apiKeyHelper` 等の資格情報ヘルパーを含む）。対象は user / project / local / managed。検知した設定ファイル変更ごとに [`ConfigChange` フック](https://code.claude.com/docs/en/hooks#configchange)が走る（MDM や claude.ai コンソールから届く managed 設定では走らない。それらはスケジュールに従って実行中セッションへ届く）。

**セッション途中で新規作成した設定ファイル**も、そのフォルダがセッション開始時に存在していればロードされる。**プロジェクトの `.claude/` フォルダについては、同一セッション中にフォルダごと作成した場合でもロードされる**。
...
| `advisorModel` | Advisor ツール（実験的）のモデル設定。メインモデルより強力なモデルをアドバイザーとして併用し、方針決定・エラー行き詰まり・完了宣言前などの要所で Claude が相談する。`/advisor` コマンド / `--advisor` フラグでも設定可。Anthropic API 専用（Bedrock / Vertex / Foundry 不可）。アドバイザーはメインモデル以上の能力が必要。**ペアリング検証は2段階（2026-09-15 リファレンス改訂で明文化）**: ①Claude Code がリクエスト前に検証し、表で「rejected」の組み合わせはアドバイザーを付けずに送信（`/advisor` 出力と通知で表示。サブエージェント自身のモデルがペアリングを満たせばそちらでは使われる）、②API 側でも再検証され、クライアントが通す一部の組み合わせ（例: メイン Sonnet 5 + アドバイザー Opus 4.6、メイン Opus 5 + Opus 4.7/4.8、メイン Fable 5.1 + Fable 5）は**全リクエストが API エラーで失敗**する（`/advisor` で変更するまで復旧しない）。主な許容ペア: Opus 5.5 / Opus 5 メインは Fable / Opus 5 以降のみ、Opus 4.7/4.8 メインは Fable / Opus 4.7 以降、Fable 5.1 メインは Fable 5.1 のみ（詳細表: https://code.claude.com/docs/en/advisor ）。組織の `availableModels` が除外するモデルの保存済みアドバイザーは呼び出されない。現在のメインモデルが非対応でもアドバイザーは保存され、対応モデルへ `/model` 切替後に有効化される（2026-09-22 ドキュメント改訂） |
| `effortLevel` | エフォートレベル。設定ファイルで指定できるのは `low` / `medium` / `high` / `xhigh` の4段階（`max` は設定ファイル不可でセッション限定、`ultracode` は別キー `ultracode` を使う）。**v2.1.251 以降は「保存済みレベルを持たないモデルに対する既定値」という位置づけ**に変わり、`/effort` は本キーではなく `modelSettings` に書き込む。同一設定ファイル内ではモデル別 `modelSettings` エントリが本キーより優先される。スキル / サブエージェントの frontmatter `effort` は当該実行中のみセッションレベルを上書きする（環境変数は上書きしない）。Managed 設定に置いても**強制ではなく既定値**で、`/effort` や `--effort` でセッション単位に変更可能。モデル別対応レベル: Fable 5 / Opus 5 / Sonnet 5 / Opus 4.8 / Opus 4.7 は `low`〜`max`、Opus 4.6 / Sonnet 4.6 は `xhigh` を除く4段階（未対応レベル指定時は直下の対応レベルにフォールバック）。既定は `high`（Opus 4.7 のみ `xhigh`。組織が[組織既定モデル](https://code.claude.com/docs/en/model-config#organization-default-model)に既定 effort を設定している場合はそのモデルでその値が既定になる）。**v2.1.280 での適用範囲変更**: user 設定（`~/.claude/settings.json`）の本キーは「per-model 化以前に `/effort` が書いていた旧形式」の扱いとなり、**Opus 5 / Fable 5.1 以前のモデルにのみ適用**される。Opus 5.5 以降の新モデルは本キーを無視し、`/effort` 等で `modelSettings` にレベルが保存されるまで各モデル既定で動く。project / local / managed / `--settings` の本キーは従来どおり全モデルに適用 |
| `modelSettings` | **v2.1.251 で追加**。モデルごとに effort レベルを保存するオブジェクト。型は「モデル名 → `effortLevel`（`"low"` / `"medium"` / `"high"` / `"xhigh"`）フィールド、`maxEffortLevel` フィールド（v2.1.267 で追加。そのモデルの effort 上限）、またはその両方を持つオブジェクト」。既定は未設定。インタラクティブセッションで `/effort low|medium|high|xhigh` を実行するか `/model` ピッカーの effort スライダを動かすと、Claude Code が**使用中モデルのエントリとして user 設定に自動で書き込む**ため手で編集する機会は少ない。キーは `claude-opus-5-5` のような正規名で書かれ、そのモデルのエイリアス・日付サフィックス付き ID・`[1m]` 付き ID・認識済みプロバイダ固有 ID が同じエントリにマッチする。VSCode 拡張のモデルピッカーでの effort 選択も同様に保存される（v2.1.280 ドキュメント改訂で明文化）。`/effort auto` で使用中モデルのエントリのみクリアされる。**優先順位**: 同一設定ファイル内ではモデル別エントリ > `effortLevel`。ファイル間ではモデルごとに独立して解決され、「そのモデルのエントリまたは `effortLevel` を設定している最上位の設定ファイル」が決める（したがって managed の `effortLevel` は user 設定の保存済みレベルに勝つ）。`ultracode` はこれらすべてに優先する。スコープ: Any file |
| `maxEffortLevel` | **v2.1.267 で追加**。セッションが使える [effort レベル](https://code.claude.com/docs/en/model-config#adjust-effort-level)の**上限**を設定する（下位レベルは引き続き選択可）。上限を超える指定は上限値で実行される — `/effort`・`/model` ピッカー・`--effort`・`CLAUDE_CODE_EFFORT_LEVEL`・モデル既定値に加え、**スキル／サブエージェントの `effort:` frontmatter も対象**（2026-09-13 リファレンス改訂で明文化）。**Claude Code がリクエスト前にクライアント側で適用するため、Amazon Bedrock / Google Cloud's Agent Platform / Microsoft Foundry でも効く**（従来の組織 effort 制限は Enterprise プラン限定だった）。型は `"low"` / `"medium"` / `"high"` / `"xhigh"` / `"max"` の文字列（`"max"` は実質キャップなし）。既定は未設定。スコープ: Any file — managed に置けば組織強制になるが、**複数スコープが設定した場合は最も低いキャップが適用される**ため、ユーザーは自分でさらに厳しくはできても緩めることはできない。モデル別に指定する場合は `modelSettings.<model>.maxEffortLevel` に置く（そのエントリは同一設定ソース内でのみトップレベル値を置き換える。`"max"` を置けばそのソースのキャップから除外できるが、他ソースのキャップは適用される）。組織の effort 制限と併用時は**低いほうが適用**。`ultracode: true` でも `xhigh` 未満のキャップがあればセッションはキャップ値で走り **ultracode は無効化**され、Claude は自発的にワークフローを計画せず `/effort` に `ultracode` も出なくなる |
...
| `managedMcpServers` | （Managed のみ、**v2.1.259**）組織が HTTP / SSE の MCP サーバーを全ユーザーへ配布する。エントリの形は `.mcp.json` と同じだが、**ローカルコマンドを起動する `command` 形式のエントリはスキップされる**。**2026-09-10 時点で公式 settings リファレンス / managed MCP ページに正式収載**（要 v2.1.259 以降）。スコープは Managed のみ（user / project / local に書くと警告付きで無視される）。ユーザーは自分で追加したサーバーを保持したまま、組織提供分は編集・削除できない。managed ソースが複数ある場合はサーバー名を全ソースから合成し、**同名は上位ソースのエントリ全体**が採用される。`allowedMcpServers` の allowlist からは除外される（`deniedMcpServers` は効く） |
| `statusLine` | カスタムステータスライン設定。v2.1.153 でステータスラインコマンドに `COLUMNS` / `LINES` 環境変数（端末サイズ）が渡されるようになった |
| `fileSuggestion` | `@` ファイル補完カスタムコマンド |
| `outputStyle` | 出力スタイル設定。組み込みは Default / Proactive / Explanatory / Learning に加え **Concise**（前置き・ナレーションを省き結果から書き出す。作業の徹底度は変えない。v2.1.237 追加）。`/config` の Output style 行から選択すると `.claude/settings.local.json` に保存される。**v2.1.251 以降、セッション中に切り替えると次のメッセージから新しいスタイルが適用される**（スタイルはシステムプロンプトの一部なので、その 1 回だけプロンプトキャッシュが再構築される）。v2.1.251 より前は `/clear` かセッション再起動が必要だった。なお**ターミナルではスタイルファイルを起動時に読む**ため、セッション中にファイルを新規作成・編集した場合は再起動が必要。切替は `/output-style <style>` コマンドでも可能（v2.1.269 以降は非対話モード・Agent SDK・Remote Control でも動作。Remote Control では組み込みスタイルのみ）。値は**大文字始まりで大文字小文字を区別**（`explanatory` と書くと Default 扱い。`/output-style` コマンド自体は大文字小文字を無視）。カスタムスタイルの frontmatter は `name` / `description` / `keep-coding-instructions`（`true` で Claude Code 組み込みのソフトウェアエンジニアリング指示を維持）/ `force-for-plugin`（**プラグイン同梱スタイル専用**。`true` でプラグイン有効時に自動適用され、ユーザーの `outputStyle` 設定を上書き。複数プラグインが設定した場合は最初にロードされたものが勝つ）。プラグインは `output-styles/` ディレクトリでスタイルを同梱できる。置き場所は user `~/.claude/output-styles` / project `.claude/output-styles`（ネスト可。同名は作業ディレクトリに最も近いものが勝つ）/ managed 設定ディレクトリ内 `.claude/output-styles` |
...
| `workflowSizeGuideline` | Dynamic workflow のサイズガイドライン（advisory）。設定時は `/config` の該当行が非表示（v2.1.219）。`small` = 5未満 / `medium` = 10未満（v2.1.271 で15→10に縮小）/ `large` = 50未満。デフォルトは medium、ただし **Pro プランでは v2.1.271 以降 small** |
...
| `strictKnownMarketplaces` | プラグインマーケットプレース許可リスト。v2.1.232 で `allowedMarketplaces` が別名として受理される。**v2.1.282 以降、埋め込みホストの親設定（parent settings）が供給する本キーはブロックされ、適用中の managed ソースのリストが強制される**（親のリストが適用されるのは managed ソースがどこにも設定していない場合のみ） |
| `wslInheritsWindowsSettings` | （Managed のみ）WSL on Windows が Windows 側の managed settings を継承（v2.1.118） |
| `blockedMarketplaces` | （Managed のみ）マーケットプレースブロックリスト。v2.1.232 で素のリポジトリ URL に対する url 型エントリが、CLI が git clone と分類した場合もブロックを継続。**v2.1.282 以降、埋め込みホストの親設定が供給するブロックリストは managed ソースのブロックリストに追加で適用される**（allowlist 系と異なりブロック方向は加算） |
...
| `allowedChannelPlugins` | （Managed のみ）チャンネルプラグイン許可リスト（v2.1.84）。型は `marketplace` / `plugin` の2フィールドを持つオブジェクトの配列。**v2.1.267 から `"plugin@marketplace"` の文字列形式も受け付ける**（例: `"telegram@claude-plugins-official"`。同等のオブジェクトとして扱われる）。**v2.1.266 以前は文字列が1つでも混ざると `allowedChannelPlugins` の値ごと拒否する**ため、旧バージョンが混在する環境ではオブジェクト形式を使う。不正値のときの挙動は v2.1.267 で変更され、キーごと破棄（＝全許可）ではなく**空の許可リストとして強制**するようになった（個別エントリのみ不正ならそのエントリだけ除去して残りを強制） |
...
| `skillOverrides` | スキル単位の表示制御。`off`（モデルと `/` から非表示）/ `user-invocable-only`（モデルから非表示）/ `name-only`（モデルには名前のみ提示し説明を出さない）。列挙されないスキルは `"on"` 扱い。v2.1.129 で実装が修正され機能するように。v2.1.199 以降、`"off"` は端末の `/` メニューに加えて **Remote Control クライアント・Agent SDK に広告されるコマンド一覧からも隠す**（フルネーム指定の呼び出しも `skillOverrides` エラーを返す）。プラグインスキルには適用されない（`/plugin` で管理する）。**エイリアスをキーにした場合の扱い（v2.1.260 以降）**: managed 設定または `--settings` で渡したファイルでは、バンドルスキルのエイリアス（`/doctor` に対する `checkup` 等）をキーにしたエントリが**その背後のスキルに適用される**。ただしエイリアス経由では制限を強める方向にしか働かず、より見えるようにはできない。managed 設定でスキル自身の名前のエントリも設定されている場合はそちらが優先。**user / project / local 設定ではスキル名にのみマッチ**し、`review` のエントリはバンドルの `/code-review` にはエイリアス経由で適用されない |
...
| `allowClaudeInChromeWithManagedMcp` | （Managed のみ）排他的 `managed-mcp.json` 環境でも `claude --chrome`（Claude in Chrome）の併用を許可する。未設定時に Chrome がブロックされた際のエラーメッセージにも本キー名が案内される（v2.1.282） |
...
| `sandbox.excludedCommands` | サンドボックス外で実行するコマンドの列挙。**既定は未設定＝除外なし**。`pbcopy` / `xclip` / `wl-copy` はサンドボックス内からシステムクリップボードに届かないことがあるため、クリップボードに渡したい場合は `/copy`（Claude Code プロセス側から書き込む）を使うか、これらを本キーに追加する。**v2.1.282 以降、managed 設定または `--settings` が `allowUnsandboxedCommands: false` を設定している場合、または managed `allowManagedDomainsOnly: true` の場合、project / local 設定由来の本キーのエントリは無視される**（リポジトリが厳格サンドボックスを迂回できないように） |
...
| `fastMode` | 利用可能なセッションで Fast モードを有効化（トークン単価は上がるが応答が速い。反復開発・ライブデバッグ向け）。`/fast` が `~/.claude/settings.json` に `fastMode: true` を書き込み、再度実行すると削除する。**Opus 5.5 / Opus 5 / Opus 4.8 でのみ動作**（v2.1.280 で Opus 5.5 追加）し、他モデルから ON にすると Opus に切り替わり、非対応モデルに切り替えると OFF になる。`CLAUDE_CODE_DISABLE_FAST_MODE` が優先。managed 設定が `disableFastMode` を設定している場合、非対話・VSCode・クラウドセッションでの `/fast on` は拒否される（v2.1.280） |
...
| `modelPricing` | （Managed のみ、v2.1.242 以降）組織の契約単価・割引倍率をコスト表示に反映させ、定価の代わりに使う。適用先は `/usage`・ステータスライン・Agent SDK の `total_cost_usd`・`--max-budget-usd`・OpenTelemetry のコストメトリクス／イベント。単価は管理者が自分で書く（契約や Console からは読まない）。任意の `multiplier`（0 超 10 以下、全コストに乗算。1 未満は割引、1 超はマークアップ＝社内チャージバック用。1 超は v2.1.271 以降で、旧バージョンは警告付きで無視し残りを適用）と任意の `overrides`（モデルID → `input` / `output` / `cacheRead` / `cacheWrite` の百万トークンあたり USD、4 つとも必須、各 0〜10000）を持つ。fast モード加算や US-only 推論単価は加算されない。行のキーが組み込みモデルIDなら、そのモデルの日付付きスナップショットIDやプロバイダ固有IDすべてに適用され、それ以外のキー（ゲートウェイのエイリアス等）はその ID 限定。Bedrock のアプリケーション推論プロファイルは解決後のモデルの行が適用される。パースできない行・`multiplier` は落として残りを使う。user / project / local / `--settings` / Windows HKCU では無視。server-managed settings 経由の場合、そのセッションの設定取得が完了するまでは定価表示 |
...
> **プラン別の既定モデル（2026-09-22 / v2.1.280 時点）**: **全プラン（Pro / Max / Team / Enterprise）と Anthropic API / Claude Platform on AWS / Bedrock / Vertex で Opus 5.5**（`claude-opus-5-5`。1M コンテキスト、$4/$20 per Mtok、キャッシュ読み $0.20/Mtok。thinking 常時オンで無効化不可）。Microsoft Foundry のみ Sonnet 4.5。v2.1.280 より前は Pro / Team Standard が Sonnet 5、Max / Team Premium / Enterprise / API が Opus 5 だった。`opus` エイリアスの解決先も v2.1.280 で Opus 5 → Opus 5.5 に変更（ピン留めするには完全 ID `claude-opus-5-5` か `ANTHROPIC_DEFAULT_OPUS_MODEL`）。
...
| `CLAUDE_CODE_PLUGIN_DIRS` | セッションにロードするプラグインディレクトリの列挙（v2.1.280 以降）。各パスは `--plugin-dir` フラグと同じ扱いでロードされ、`--plugin-dir` 指定分に**追加**される。複数パスは Unix `:` / Windows `;` 区切り。**絶対パスまたは `~` 始まりのみ**（相対パスはスキップ）。フラグを渡せない環境（IDE 拡張・ラッパー経由等）でのプラグインロード手段。project / local 設定の `env` からは設定不可 |
...
| `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS` | **v2.1.260 で削除され no-op**。従来はサブエージェントが起動したバックグラウンドシェルコマンドの寿命上限（既定 60 分）だった。現在サブエージェント所有のバックグラウンドコマンドに時間制限はなく、フォアグラウンド実行中のサブエージェントが最終応答を返した時点で終了するのみ |
...
| `CLAUDE_CODE_GOAL_CHECKIN_MINUTES` | `/goal` 実行中、バックグラウンド作業がゴール評価を待たせ続けたときに Claude へ状況確認を促すまでの分数（デフォルト `30`、`0` で無効、最大 `10080`＝1週間。プレーンな整数以外は未設定扱い）。Claude Code は実行中タスクを列挙し、出力の確認・進捗中なら待機継続・停滞中なら修正か停止を依頼する（v2.1.234）。v2.1.236 でアイドルセッションのゴールがバックグラウンド作業の背後で停滞している場合、ユーザーの復帰を待たず 30 分後（以降 1h・2h）に自動チェックインするよう拡張。**`0` は v2.1.269 で追加された一時的エラー後のゴール自動リトライも無効化する** |
...
| `CLAUDE_CODE_ENABLE_TODO_TOOLS` | `1` で Todo / タスク追跡ツール（`TaskCreate` / `TaskGet` / `TaskUpdate` / `TaskList` / `TodoWrite`）を全モデルで提供する。**v2.1.268 で提供条件が反転し、既定提供は Claude 3.x / Opus 4.0–4.7 / Sonnet 4.0–4.6 / Haiku 4.5 のみ**になった（LLM ゲートウェイ経由のカスタムモデル名など未知のモデル ID も非提供側）。これらのツールに依存するハーネスは明示設定が必要 |
| `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` | WebFetch のページダウンロード期限（ms、リダイレクト追跡込み）。既定 `300000`（5分）、`0` で無効化。素の数字のみ受理。v2.1.268 以降 |
| `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` | Workflow ツールの実行あたり並列エージェント上限を引き上げる（`1`〜`256`）。推論律速のファンアウト向け。v2.1.269 以降 |
| `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` | LLM ゲートウェイの `/v1/models` 発見タイムアウトを延長（既定 3 秒）。v2.1.269 以降 |
| `OTEL_METRICS_INCLUDE_REPOSITORY` | `true` で OpenTelemetry のメトリクス・イベントに `vcs.*` リポジトリ属性を付与（既定は除外）。`OTEL_LOG_TOOL_DETAILS` 併用時、コミットイベントに `vcs.ref.head.*` が付く。v2.1.269 以降 |
| `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING` | `0` で、リモート／ヘッドレスセッションがバックグラウンドエージェント実行中でも「入力待ち」を報告する旧挙動へ戻す（v2.1.269 で既定挙動が修正された） |
...
| `CLAUDE_JOB_DIR` | **Claude Code が各バックグラウンドセッションに自動設定する**変数（`~/.claude/jobs/<id>`）。当該セッションが実行するシェルコマンドに継承される。スクラッチファイルは `$CLAUDE_JOB_DIR/tmp` に置く（Claude の `Write` / `Edit` はそこでは権限プロンプトを出さず、セッション削除時にディレクトリごと消える） |
