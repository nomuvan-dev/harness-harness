...
最終更新: 2026-09-27（**v2.1.283**（2026-09-25）を反映。ハーネス観点の目玉は **`availableModelsMatch` / `deniedModels` managed 設定**（前者は `"exact"` 指定で `availableModels` のモデルIDエントリを記載バージョンのみに限定し新リリースの自動許可を止める（既定 `"prefix"`。ファミリーエイリアスは従来どおりファミリー全体を許可）。後者は `availableModels` が許すモデルも個別ブロックし `/model` ピッカーから隠す（マイナーバージョンなしのIDは後続マイナーもブロック。Opus 5 だけなら `claude-opus-5-0`）。ともに Managed 専用で他スコープでは警告付き無視）、**`/doctor prompt-audit`（`/checkup prompt-audit`）**（CLAUDE.md・スキル・エージェント・コマンドを旧モデル向けプロンプトパターンについて監査。stale パス・stale コマンド・矛盾する指示ファイルをレポート先頭に表示）、**2.1.282 の `claude-ai` 名前空間予約のリバート**（`claude-ai` 名のスキル・コマンド・ワークフロー・MCP サーバーのスキル / プロンプトは再びロードされ、`Skill(claude-ai:*)` ルールは通常のプレフィックスルールに戻った。`anthropic-skills:` の保護は維持）、**`Skill(anthropic-skills:<name>)` deny が Claude Desktop 経由のプラグイン形態にも適用＋`Skill(skill:<name>)` deny がスキルのエイリアス・表示名にも一致**、**サードパーティプロバイダ / テレメトリ無効の対話セッションが permission mode 未設定時 auto モードで開始**（`permissions.defaultMode` で上書き可）、**自動 armed の artifact watch の3.5時間無活動タイムアウト**。ほかに gateway の `x-claude-code-prompt-id` ヒントヘッダ / `load_test_mode`（ロードテスト用の送信なし署名込みリクエスト構築）/ Bedrock Mantle 向け `mantle` provider、`OTEL_LOG_TOOL_CONTENT=1` 時の `tool.output` span イベントに MCP / WebFetch / WebSearch 出力を追加、`--system-prompt` / `--append-system-prompt` のテキスト形式と `-file` 形式の併用可、MCP ツールが返す画像のファイル保存（Bash / Read で開ける）、`/context` への MCP サーバー instructions 行の計上、plugin validate / uninstall / `installed_plugins.json` の堅牢化修正多数、Windows PowerShell ツールの `cmd /c rd` 等によるドライブルート削除防止、SDK セッションのツール結果取りこぼし修正、起動・初回応答レイテンシ改善。前回: **v2.1.282**（2026-09-24）を反映。ハーネス観点の目玉は **project / local 設定の `env` から OpenTelemetry エクスポート系変数（`CLAUDE_CODE_ENABLE_TELEMETRY`・`OTEL_LOG_*` 等）を無視する変更**（無視・オフにされた変数は起動時通知と `/status` / `claude doctor` に一覧。settings-example からもテレメトリ設定例が削除され「テレメトリは managed / user 設定に置く」方針が明文化）、**`anthropic-skills:` / `claude-ai:` 名前空間の保護**（この名前空間のスキルフォルダ・コマンド・ワークフローはロードされず、`Skill(anthropic-skills:*)` allow ルールは同期スキルのみに一致。なりすまし防止）、**`maxProseWidth` 設定**（ワイド端末での散文表示幅制限）、**auto モードのサーバー側分類器が直接 Anthropic API 接続ではテレメトリ無効でも既定に**（`CLAUDE_CODE_AUTO_MODE_SERVER=0` でオプトアウト）、**厳格サンドボックス下で project / local の `sandbox.excludedCommands` を無視**、**`allowClaudeInChromeWithManagedMcp`**（排他的 managed-mcp.json と `--chrome` の併用許可）、**managed 設定の堅牢化修正多数**（boolean ロックキーの誤記値でもロック適用・ネスト一部無効でも残りを適用・`allowManagedPermissionRulesOnly` 下で skills / commands の `allowed-tools` 自己事前承認を遮断）、**Bash 権限ルールの中間 `:*` パターンが設定ファイルからも機能するように修正**。gateway に `store.readiness_grace_seconds` 追加。あわせて**プラグインドキュメントの大再編**: 単一ページ群（plugins / discover-plugins / plugins-reference / plugin-marketplaces 等）が `/docs/en/plugins/` 以下の約20ページ（overview / install / create / components / manifest-reference / marketplace-reference / org / security / loading / troubleshooting 等）に分割された（specs 内の参照 URL も更新済み）。ドキュメント改訂では **claude.ai 管理コンソール由来の `strictKnownMarketplaces` は親設定（埋め込みホスト）供給分をブロックし managed を強制・`blockedMarketplaces` は加算適用**（v2.1.282+）、**send-now キー（ctrl+enter）の挙動変更**（v2.1.281 以降、バックグラウンド化可能な作業はターン中断でなくバックグラウンドへ移動して続行）、**Projects のスレッドがクラウド専用でなくローカル実行too可能に**（記述変更）、**Agent SDK troubleshooting の症状別インデックス化**が入った。前回: **v2.1.281**（2026-09-23）を反映。ハーネス観点の最大の目玉は **AGENTS.md 直接読み込みのフィーチャーフラグ依存解消**：Bedrock / Vertex / Foundry・テレメトリ無効環境でも AGENTS.md をプロジェクト指示として直接読めるようになった（v2.1.277 以来の「未対応環境」制約が解消。mapping/ の変換要否判断に影響）。ほかに **settings.json の `"attribution": false`**（コミット / PR の attribution 行を全て非表示。旧バージョン CLI はこのキーを含む settings ファイル自体をスキップするため、バージョン混在環境で共有するファイルではオブジェクト形式を維持する）、**コマンド置換のみを対象とする再帰 rm（`rm -rf "$(pwd)"` 等）が auto / `--dangerously-skip-permissions` / Bash allow ルール下でも確認を要求**（`CLAUDE_CODE_DISABLE_SUBSTITUTION_RM_PROMPT=1` で無効化）、**`--setting-sources`（SDK `settingSources`）が派生セッション（チームメイト / `/bg` / `claude agents` / `--worktree --tmux`）に引き継がれる修正**、**`claude plugin validate` の MCP サーバー検査**（ロード時に黙って捨てられる `.mcp.json` エントリ・未宣言 `${user_config.*}` 参照・非セキュア URL を報告）、**Claude apps gateway の Bedrock `assume_role`（STS 経由の別アカウントロール、開発者ごとセッション可）/ Bedrock `guardrail: {id, version}` / `telemetry.resource_attributes` / desktop ポリシーの新キー（`blockReadsOutsideWorkingDirectories`・`disableBypassPermissionsMode`）**、**MCP URL モード elicitation（2026-07-28 プロトコル。サーバーがブラウザベースのフローを要求可能）**、**`--agents` の JSON ファイルパス指定（`-p` 併用）**、**auto モード拒否の扱い改善**（拒否をコマンド単発でなく結果への拒否として扱わせる）、**`/insights` の auto モード推奨表示**。NUL バイトを含む権限ルールがワイルドカード化していた穴の修正、macOS `/.vol` 等経由の承認前パス読み取り修正、`claude --bg` の workspace trust 要求などセキュリティ修正多数、resume / プロンプトキャッシュ / プロキシ切断まわりの大量の信頼性修正も入った。あわせてドキュメント改訂で **クラウドセッションが research preview 表記を外れ Pro / Max / Team で正式提供**、**次プロンプト提案のコスト明記**（セッションと同じモデルへの短いバックグラウンドリクエストとしてプラン使用量 / API コストに計上。キャッシュ読み中心で少額）、**claude.ai 管理コンソールの `strictKnownMarketplaces` / `blockedMarketplaces` が claude.ai 上の git リポジトリからのマーケットプレース追加にも適用**が明文化された。前回: **v2.1.280**（2026-09-22）を反映。ハーネス観点の最大の目玉は **Claude Opus 5.5（`claude-opus-5-5`）追加**：Opus ファミリーの新既定（`opus` エイリアス・`default` の解決先）。1M コンテキスト、$4/$20 per Mtok（キャッシュ読み $0.20/Mtok）。**Pro / Team Standard の既定モデルも Sonnet → Opus に変更**され、全プラン・Anthropic API / AWS / Bedrock / Vertex で `default` = Opus 5.5 に統一。Opus 5.5 は thinking 常時オン（Fable 同様に無効化不可）で、fast モード対応。effort まわりの挙動変更あり：**per-model 化以前に保存された旧 `effortLevel`（user 設定）は Opus 5.5 以降の新モデルには適用されず**、レベルを保存するまで各モデルの既定で動く（project / local / managed / `--settings` の `effortLevel` は従来どおり全モデル適用）。ほかに **`CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`**（MCP ツール説明の 2,048 文字上限を変更）、**`PermissionRequest` イベントで agent フックがスキップされる**明文化（command / http / mcp_tool / prompt のみ対応）、**バックグラウンドサブエージェントで LSP 使用可能に**、**symlink 経由書き込みの権限判定をリンク先パスで行う修正**、**`/memory` / `/context` に直接読み込みの AGENTS.md が表示されるように**、**ダイアログの `y` / `n` キー既定バインド廃止**（Enter / Esc に。`keybindings.json` で復元可）、**予約マーケットプレース名を偽装するマーケットプレースの追加拒否**、**Claude Docs（旧 claude-projects）対応の VSCode 拡張機能強化**（/status・/sandbox・/chrome・/export・/skills・/plan ダイアログ追加）。あわせてドキュメント改訂で **managed 設定の不正値フィールドごとのフェイルセーフ挙動表**（allowlist 系は空リスト強制、`allowManagedHooksOnly` 等は `true` 扱い）、**クラウドセッションはリポジトリ宣言プラグイン（`enabledPlugins` / `extraKnownMarketplaces`）を一切ロードしない**方針変更（claude.ai アカウント同期プラグインを使う）、**サブエージェント frontmatter のモデルエイリアスがメイン会話と同ファミリーの場合メイン会話の正確なモデル（`[1m]` 含む）に解決される**仕様、**同期スキルはダウンロード専用**（ローカル編集は次回同期で上書き）、**Invisible characters のプロンプト除去仕様**（貼り付けテキストの不可視 Unicode 文字を Enter 時に除去し再確認を要求）が明文化された。前回: **v2.1.277**（2026-09-18）と **v2.1.278**（2026-09-19）を反映。ハーネス観点の最大の目玉は **AGENTS.md ネイティブサポート**：CLAUDE.md がないプロジェクトでは AGENTS.md をプロジェクト指示として直接読む（`/config` の「Project instructions」または `pluginConfigs` の `agents-md@builtin` で `claude-md-and-agents-md` / `claude-md` 等に変更可。Bedrock / Vertex / Foundry は未対応）。ハーネス設計への影響大：AGENTS.md 単一ファイルで Claude / Codex 両対応が可能になった（mapping/ 参照）。ほかに **TaskOutput ツール削除**（Read で出力ファイルを読む方式へ。`taskOutputMaxChars` / `TASK_MAX_OUTPUT_LENGTH` は無効化）、**サブエージェント結果のヘッダ付きフレーミング**（結果内テキストがセッション指示として通らないように）、**ゲートウェイの `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` と upstream `headers:` マップ**。2.1.278 では **auto モードのサーバー側分類器が Claude API / Enterprise / Bedrock / Vertex / Foundry / ゲートウェイで既定化**（分類器オーバーヘッド非課金、`/status` に「Auto mode server」行）。あわせてドキュメントに **Projects**（claude-projects.md 新設。1つの会話で複数クラウドセッション（スレッド）をClaudeが調整。Pro/Max パブリックベータ・段階ロールアウト）、**クラウド環境の個人環境共有**（Owner が「Who can use it」から組織共有可）、**settings-reference のビルトインプラグイン `@builtin` サフィックス**、**InstructionsLoaded フックが AGENTS.md 直接読み込みでは発火しない**注記が追加された。前回: **v2.1.275**（2026-09-17）と **v2.1.276**（2026-09-18）を反映。ハーネス観点の目玉は **claude.ai スキル / プラグイン自動同期の実装本体リリース**（`syncClaudeAiSkills` / `syncClaudeAiPlugins` でオプトアウト）、**npm ソースプラグインの `--ignore-scripts` 取得＋整合性検証**（install スクリプト非実行のサプライチェーン対策）、**`/plugin install --marketplace`**、**send-now キー（ctrl+enter）**。2.1.276 はゲートウェイ環境での `advisor_20260301` 400 リグレッション修正のみ。あわせてドキュメント改訂で **フック入力への `mcp_server` オブジェクト追加（v2.1.274+、`source` で提供元を判別）**、**MCP の 403 insufficient_scope 時の再認証フロー明文化**、**組織が claude.ai の Skills を無効化した場合の同期スキル削除（`.trash/` 退避）**、**チームメイトの自動復帰とフォルダ信頼要件**が記載された。前回: **v2.1.274**（2026-09-17）を反映。ハーネス観点の目玉は **MCP v2 ランタイム＋2026-07-28 ネゴシエーションの既定が Bedrock / Vertex / Foundry・テレメトリ無効環境にも拡大**、**プラグイン/マーケットプレース clone の Git LFS 既定スキップ（`skipLfs` は無効化）**、**`CLAUDE_CODE_MCP_STARTUP_WAIT_MS`**、**`"type": "sdk"` MCP エントリの警告付きスキップ**。あわせてドキュメント改訂で **claude.ai スキル同期がサインイン済みターミナルセッションで自動化**（`CLAUDE_CODE_SYNC_SKILLS` は非対話実行での待機用に格下げ）され、**`syncClaudeAiPlugins` 設定が新設**、**複数リポジトリのクラウドセッションはリポジトリの `.claude/settings.json` の hooks / permission を読まない**ことが明文化された。前回: **v2.1.273**（2026-09-15）を反映。ハーネス観点の目玉は **LLM ゲートウェイ向けヒントヘッダ（`CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`）**、**Remote Control セッションの Claude アプリからのフォーク**、**Bedrock / Vertex / Foundry の auto モードがローカル分類器既定に**（`CLAUDE_CODE_AUTO_MODE_SERVER=1` でサーバー側へ）、**2.1.268 の「解析不能 Bash 行への Read / Edit deny 適用」リバート**。あわせて公式ドキュメント全体で「Claude Code on the web」の呼称が「cloud sessions（クラウドセッション）」へ改称された。前回: **v2.1.271**（2026-09-14）と **v2.1.272**（2026-09-15）を反映。2.1.272 はバグ修正のみ。2.1.271 のハーネス観点の目玉は **サブエージェント frontmatter の `omitClaudeMd`**（CLAUDE.md を読ませないサブエージェント定義が可能に）、**Monitor の watch に常時デッドライン導入（`persistent` オプション廃止）**、**auto モード＋サンドボックスでのコマンド単位 `allowed_domains`**、**リモートMCPサーバーへの認証情報環境変数の展開遮断**（`${ANTHROPIC_API_KEY}` 等は空として読まれる）、**dynamic workflow の使用上限到達時の自動一時停止＋medium ガイドライン 15→10 縮小（Pro は既定 small）**。）

---

## v2.1.283 (2026-09-25)

**モデル統制の強化（availableModelsMatch / deniedModels）・prompt-audit・claude-ai 名前空間予約のリバート**

### 新機能

- **`availableModelsMatch`（Managed のみ）**: `availableModels` のモデルIDエントリのマッチ方法を選択。既定 `"prefix"` はエントリを1セグメント延長した後続モデルIDも許可（`claude-opus-5` → Opus 5.5 も可）。`"exact"` は記載バージョンのみ許可（そのバージョンの日付付きIDは含む）し、新リリースはリストに追加するまでブロック。ファミリーエイリアス（`opus` 等）は `"exact"` でもファミリー全体を許可し、`best` / `opusplan` / `default` エントリは無視。`"exact"` ではリストがモデルかファミリーを1つでも指定していれば Default オプションもリスト内に限定される
- **`deniedModels`（Managed のみ）**: 特定モデルをブロック（`availableModels` の有無・許可に関わらず適用）。ブロックされたモデルは `/model` ピッカーから隠され、`availableModels` が強制される全箇所で選択不可。Default オプションもブロックモデルでは動かない。ファミリーエイリアスはファミリー全体、モデルIDは全表記（日付付き・プロバイダ固有ID含む）をブロック。マイナーバージョンなしのID（`claude-opus-5`）は後続マイナー（Opus 5.5）もブロックするため、Opus 5 単体は `claude-opus-5-0` と書く
- **`/doctor prompt-audit`（`/checkup prompt-audit`）**: CLAUDE.md・スキル・エージェント・コマンドを旧モデル向けプロンプトパターンについて監査。stale パス・stale コマンド・矛盾する指示ファイルをレポート先頭に表示し、Claude Code がドキュメントする thinking キーワードは温存
- **gateway**: `x-claude-code-prompt-id` ヒントヘッダ追加（`CLAUDE_CODE_GATEWAY_HINT_HEADERS=1` でオプトイン。1ユーザープロンプトに属するリクエストをゲートウェイがグループ化可能）、`load_test_mode` ブロック（リクエストを構築・署名するが上流に送らず定型応答。デプロイのロードテスト用）、Bedrock Mantle エンドポイント向け `mantle` upstream provider
- **OpenTelemetry**: `OTEL_LOG_TOOL_CONTENT=1` 時の `tool.output` span イベントに MCP ツール・WebFetch・WebSearch の出力を追加
- **stream-json**: `system/init` の `plugin_errors` の `--plugin-dir` ロード失敗エントリに `path` 追加

### 変更

- **2.1.282 の `claude-ai` 名前空間予約をリバート**: `claude-ai` 名のスキル・コマンド・ワークフロー・MCP サーバーのスキル / プロンプトは再びロードされ、`Skill(claude-ai:*)` ルールは通常のプレフィックスルールに戻った（`anthropic-skills:` の保護は維持）
- **Skill 権限ルールの拡張**: `Skill(anthropic-skills:<name>)` の deny ルールは Claude Desktop がプラグインとして配布する同スキルもブロック。`Skill(skill:<name>)` の deny はスキルのエイリアス・表示名にも一致
- **サードパーティプロバイダ / テレメトリ無効の対話セッション**: permission mode 未設定時に auto モードで開始（`permissions.defaultMode` で上書き可）
- **`--system-prompt` / `--append-system-prompt`**: テキスト形式と `-file` 形式の併用が可能に（ファイルのテキストが先）
- **artifact watch**: 自動で armed になった watch（明示要求でないもの）は3.5時間無活動で終了。再 publish / watch で再 arm
- `/ultrareview` 起動ダイアログにローカルブランチレビューが追跡ファイルの未コミット変更をアップロードしうる旨を明記
- `claude plugin eval` は git 2.31 以降を要求（古い git では拒否）
- プロンプト提案が20回連続未使用で表示頻度を低減（使用で復帰）

### 修正（ハーネス関連の主なもの）

- **plugin まわりの堅牢化多数**: `claude plugin validate` がインストール不能な plugin / marketplace 名や `outputStyles` / `themes` / `monitors` / `lspServers` の欠落・プラグイン外パスを検出、大文字小文字違いの plugin id での誤アンインストール修正、home / config ディレクトリ移動後の「cache-miss」ロード失敗修正、`installed_plugins.json` の不正レコードでの全損防止（レコード名と復旧方法を提示、再構築前にバックアップ保存）
- **MCP**: 長時間実行ツールのバックグラウンド移行後も進捗通知を表示、セッション終了時の stdio サーバー残留修正、stateless リモートサーバーの一時 404 での永続不能化修正、`/context` に MCP サーバー instructions を行として計上、MCP ツールが返す画像をファイルにも保存（Bash / Read で開ける）
- **SDK**: ターン早期終了時の deferred ツールコール / 完了済みツール結果の喪失、ワーカー再起動後の保留承認プロンプト、非ストリーミングフォールバックの `result.usage` を修正
- **モデル統制関連**: `/model` の `[1m]` 付き Sonnet 4.6 / Sonnet 5 が日付・`-v1:0` サフィックス付きIDで受理されない問題、`ANTHROPIC_DEFAULT_HAIKU_MODEL` 指定時のピッカーのハードコード表示、モデルフォールバック中に開始した dynamic workflow が全エージェントをフォールバックモデルで実行する問題を修正
- **managed 設定**: `sandbox` 設定のネスト値1つの不正で全体が無視される問題を修正（不正値のみ fail closed、残りは適用）
- **Windows**: PowerShell ツールで `cmd /c rd` / `rmdir` / `del` / `erase` によるドライブルート・ホームフォルダ等の削除を防止
- git リポジトリのサブディレクトリ起動時に auto-memory ノートへの自己編集が sensitive-file write としてブロックされる問題を修正
- テレメトリ無効時に週次 Fable 上限が `/usage` に出ない問題、`DISABLE_TELEMETRY` / `DO_NOT_TRACK` 時の Remote Control 不可を修正
- 起動・レイテンシ改善: `claude -p` / Remote の対話UI非ロード、初回応答中のパターンコンパイル並行化、preconnect 済みAPI接続の再利用

## v2.1.282 (2026-09-24)

**テレメトリ変数のリポジトリ統制排除・スキル名前空間保護・managed 設定の堅牢化**

### 新機能

- **`maxProseWidth`（settings.json）**: ワイドターミナルで Claude の散文出力の幅を制限（テーブル・コードブロックは全幅維持）
- **`allowClaudeInChromeWithManagedMcp`（managed 設定）**: 排他的 `managed-mcp.json` 環境でも `claude --chrome` の併用を許可。Chrome ブロック時のエラーが本キー名を案内
- **テレメトリ変数の可視化**: project 設定ファイル内で無視された・テレメトリをオフにしたテレメトリ変数を、起動時通知と `/status` / `claude doctor` に一覧表示
- **Claude apps gateway**: `store.readiness_grace_seconds` 追加（短時間の Postgres 障害・フェイルオーバー中も `/readyz` を ready に維持）

### 変更

- **project / local 設定の `env` から OpenTelemetry エクスポート系変数を無視**: `CLAUDE_CODE_ENABLE_TELEMETRY`、エクスポータ / エンドポイント指定、`OTEL_LOG_*` 等のコンテンツキャプチャ系（一部のオフ値を除く）。settings-reference に「Variables Claude Code ignores in `env`」節が新設され、settings-example からもテレメトリ設定例が削除された（テレメトリは managed / user 設定で統制する方針）
- **`anthropic-skills:` / `claude-ai:` 名前空間の保護**: この名前空間のスキルフォルダ・コマンドファイル・ワークフローコマンドはロードされない（同名プラグインはロードされるが名前衝突は同期スキル優先）。`Skill(anthropic-skills:*)` / `Skill(claude-ai:*)` allow ルールは claude.ai 同期スキルのみに一致。この名前の MCP サーバーはスキル / プロンプトを一覧しない（ツールは動作）
- **auto モードのサーバー側分類器**: 直接 Anthropic API 接続ではテレメトリ無効でも既定に（`CLAUDE_CODE_AUTO_MODE_SERVER=0` でオプトアウト）
- **`sandbox.excludedCommands`**: managed / `--settings` が `allowUnsandboxedCommands: false`、または managed `allowManagedDomainsOnly: true` の場合、project / local 設定のエントリを無視
- **Windows/WSL の managed 設定**: 存在するが無効・読み取り不能な admin ポリシー（HKLM / `managed-settings.json`）がある場合、ユーザー書き込み可能な HKCU / WSL `/etc/claude-code` の適用を止める（弱い側へのフォールバック防止）
- AGENTS.md サポートが Bedrock / Vertex / Foundry・LLM ゲートウェイ・テレメトリ無効環境でも動作（公式 changelog 上は v2.1.281 の記載が改訂され「Changed」として明記）

### セキュリティ / 権限修正

- managed 設定の boolean ロックキー（`disableClaudeAiConnectors` / `allowManagedPermissionRulesOnly` 等）の誤記値が無視されていた問題を修正（ロックを適用し起動時にキー名を警告）
- managed の `permissions` / `autoMode` / `worktree` / `attribution` がネスト値 1 つの不正で丸ごと無視されていた問題を修正（残りは適用）
- `allowManagedPermissionRulesOnly` 下で、リポジトリ / user / `--add-dir` のスキル・コマンド・skills-directory プラグインマニフェストが `allowed-tools` で自ツールを事前承認できていた穴を修正
- 中間 `:*` を含む Bash 権限ルールが設定ファイルからスキップされていた問題を修正（全ソースで機能。マッチ方法の起動時警告付き）
- CLAUDE.md / rules が `..` 経由の macOS `/Network` 到達 symlink や `/.vol` 形式カーネルパス経由で読まれる問題を修正

### 修正（抜粋）

- resume / continue 時の過去メッセージ変形再送・extended thinking 喪失の追加修正（即時スラッシュコマンド使用時・`--tools` 縮小時・`redacted_thinking` API エラー時のリトライ）
- コンパクション要約リクエストが拒否された場合のフォールバックモデル再試行
- 別プロセスのサインイン更新中に最大1分リクエストが失敗する問題・組織ポリシー取得のリトライ欠如を修正
- `/install-github-app` のキャンセル後にブランチ push と secret 保存が続行される問題を修正
- プラグインアンインストールが失敗時にも成功報告し保存オプション・シークレットを消す問題を修正
- 大規模セッションの resume 高速化、vim モードの多数の修正、`/artifacts` の UI 改善
- \[Claude Tag] Enterprise Grid の auto-join / 共有チャンネルまわり多数、リトライされたモデルでのスレッド移行、コスト表示の過大表示修正

### ドキュメント改訂（同時期）

- **プラグインドキュメントの大再編**: `plugins`（旧 Create plugins）・`discover-plugins`・`plugins-reference`・`plugin-marketplaces`・`plugin-dependencies`・`plugin-hints`・`plugin-relevance` が `/docs/en/plugins/` 以下の約20ページに分割（overview / install / anthropic-marketplaces / code-intelligence / security / create / components / dependencies / publish / measure / cli-hints / create-marketplace / host-marketplace / relevance / org / troubleshooting / loading / manifest-reference / marketplace-reference / cli-reference）
- claude.ai 管理コンソール由来 `strictKnownMarketplaces` は親設定（埋め込みホスト）供給分をブロックして managed を強制、`blockedMarketplaces` は親設定分が加算適用（v2.1.282+）
- send-now キー（ctrl+enter）の挙動: v2.1.281 以降、バックグラウンド化可能な作業（シェルコマンド・サブエージェント等）はターンを中断せずバックグラウンドへ移動して続行し、キューを送信（応答生成のみ・バックグラウンド化不能な場合は従来どおり中断）
- Projects: スレッドは「通常はクラウドセッション」となり、マシン固有のタスクはローカル実行も可能に（記述変更）
- Agent SDK troubleshooting が症状→担当ページの対応表付きに再構成

---

## v2.1.281 (2026-09-23)

**AGENTS.md のフィーチャーフラグ依存解消・attribution 非表示設定・セキュリティ / 信頼性修正多数**

### 新機能

- **`"attribution": false`（settings.json）**: コミットメッセージ・PR 説明の attribution 行をすべて非表示にできる。**注意**: このキーを含む settings ファイルは旧バージョン CLI がファイルごとスキップするため、バージョン混在環境で共有するファイルでは従来のオブジェクト形式を維持する
- **Claude apps gateway 強化**: Bedrock upstream の `assume_role`（STS でロールを引き受けて Bedrock を呼ぶ。別 AWS アカウント可・開発者ごとのセッション可）、`guardrail: {id, version}`（全リクエストに Amazon Bedrock guardrail 適用。全 Bedrock upstream に設定するか全く設定しないか）、`telemetry.resource_attributes`（テレメトリへの固定ラベル付与）、desktop ポリシーブロックの新キー（`blockReadsOutsideWorkingDirectories`・`disableBypassPermissionsMode`）
- **MCP URL モード elicitation**（2026-07-28 プロトコル接続）: サーバーが Claude Code にブラウザベースのフローを開くよう要求できる
- **`claude plugin validate` の MCP サーバー検査**: ロード時に黙って捨てられる `.mcp.json` エントリ・未宣言の `${user_config.*}` 参照・非セキュア URL を報告
- `/insights` に auto モード推奨を追加（最近のセッションで auto モードが処理できたはずの権限プロンプト数を推定）
- `--agents` が inline JSON に加えて JSON ファイルパスを受け付け（`-p` 併用時）。`prompt` 空も許容
- `/batch` が WorktreeCreate フックで worktree を提供する環境なら git リポジトリ外でも動作

### 変更

- **AGENTS.md 直接読み込みのフィーチャーフラグ依存を解消**: Bedrock / Vertex / Foundry・テレメトリ無効環境でも AGENTS.md をプロジェクト指示として直接読めるように（v2.1.277 以来の制約解消）。残る制約は v2.1.277 未満・ビルトイン `agents-md` プラグイン無効時・（一部ケースで）v2.1.276 以前からのアップグレード直後の初回セッションのみ
- **auto モード拒否の扱い改善**: 拒否メッセージを「そのコマンド単発への拒否」でなく「その結果への拒否」として Claude に扱わせる

### セキュリティ修正

- **コマンド置換のみを対象とする再帰 rm（`rm -rf "$(pwd)"` 等）が auto / `--dangerously-skip-permissions` / Bash allow ルール下でも確認を要求**（`CLAUDE_CODE_DISABLE_SUBSTITUTION_RM_PROMPT=1` で無効化）。dangerous-rm 検査も強化（シェル変数＋トップレベルディレクトリ名、作業ディレクトリ由来の変数、バックスラッシュのみのターゲット）
- NUL バイトを含む権限ルールがワイルドカードに展開されていた問題を修正（何にもマッチしなくなる）
- macOS の `/.vol` / `/.nofollow` / `/.resolve` 経由パス（ネットワークマウント到達可）を承認前に読んでいた問題を修正
- `claude --bg` が workspace trust プロンプト未通過ディレクトリでセッション・プロジェクトフックを起動していた問題を修正（trust を先に要求、非対話なら終了）
- リモート MCP サーバーの URL 綴り違い（大文字小文字・デフォルトポート・末尾スラッシュ）による二重接続を修正

### 修正・改善（抜粋）

- **`--setting-sources`（SDK `settingSources`）が派生セッションに引き継がれるように**: チームメイト / `/bg` / `claude agents` / `--worktree --tmux` が親の制限を継承
- resume まわり多数: 過去ターンの変形再送によるプロンプトキャッシュ / 推論履歴の喪失、巨大セッションの部分復元、ツールコール中断後の再開（結果不明を明示）、advisor 結果由来の毎ターンエラーの一回修復
- プロキシ / ゲートウェイ切断まわり多数: 途中終了レスポンスの警告なし完了表示・ツール二重実行、"Content block not found"、stop reason 喪失、`CLAUDE_CODE_RETRY_WATCHDOG` の失敗
- `mcp_tool` フック（PreToolUse 等ブロッキングイベント）が MCP サーバー接続中にスキップされていた問題を修正（接続タイムアウトまで待つように）
- 起動高速化: managed settings のネットワーク待ち解消、git 読み取り / テレメトリ / モデルアップグレードチェックの初回フレーム後送り。長大セッションの resume 高速化
- vim モードの多数の修正（`dj`/`dk`/`cw`/`.` リピート等）、`/config` / `/plugin` / `/permissions` 等 UI 修正多数
- Windows: `$TMPDIR` 書き込み失敗、同時アップデートによる `claude.exe` 消失の競合を修正

### ドキュメント改訂（2026-09-25 巡回で検出）

- **クラウドセッションが research preview 表記を外れ、Pro / Max / Team プランで正式提供に**（cloud-environments.md）
- **次プロンプト提案のコスト明記**: セッションと同じモデルへの短いバックグラウンドリクエストとしてプラン使用量 / API コストに計上（キャッシュ読み中心で少額）（interactive-mode.md）
- **claude.ai 管理コンソールで設定した `strictKnownMarketplaces` / `blockedMarketplaces` は、claude.ai 上（Cowork の Customize 含む）での git リポジトリからのマーケットプレース追加にも claude.ai 自身が適用**（settings-reference.md / managed-settings.md）

## v2.1.280 (2026-09-22)

**Claude Opus 5.5 リリース・既定モデル統一・effort per-model 移行完了**

### 新機能

- **Claude Opus 5.5（`claude-opus-5-5`）追加**。Opus ファミリーの新既定モデル（`opus` エイリアス・`default` の解決先）。1M コンテキスト、$4/$20 per Mtok（キャッシュ読み $0.20/Mtok）。thinking 常時オン（`CLAUDE_CODE_DISABLE_THINKING` / `MAX_THINKING_TOKENS=0` でも無効化不可）。fast モード対応（Opus 5.5 / 5 / 4.8）。安全性分類器つき（biology フラグは Opus 5 へ、cybersecurity フラグは Opus 4.8 へフォールバック）。Vertex 用リージョン変数 `VERTEX_REGION_CLAUDE_5_5_OPUS` 追加
- `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`: MCP ツール説明・サーバー instructions の 2,048 文字上限をセッション全体で変更
- **バックグラウンドサブエージェントが LSP ツールを使用可能に**（従来は前景のみ）
- `/memory` / `/context` に、直接読み込まれた AGENTS.md が表示されるように（従来は CLAUDE.md がインポートした場合のみ）
- \[VSCode] `/status` / `/sandbox` / `/chrome` / `/export` / `/skills` / `/plan` ダイアログ追加。800 文字超・2 改行超のペーストをマーカーで区別

### 変更

- **Pro / Team Standard の既定モデルが Sonnet → Opus に変更**。全プラン・Anthropic API / Claude Platform on AWS / Bedrock / Vertex で `default` = Opus 5.5 に統一（Foundry は Sonnet 4.5 のまま）
- **effort の per-model 移行完了**: per-model 化（v2.1.251）以前に保存された旧トップレベル `effortLevel`（user 設定）は **Opus 5.5 以降の新モデルには適用されない**。レベルを保存するまで各モデル既定の effort で動く。project / local / managed / `--settings` の `effortLevel` は従来どおり全モデル適用。Opus 4.7 / 4.8 / Fable 5 の「起動時既定 effort ホールド」も廃止され、`-p` / SDK / project / managed / per-model 設定の `/effort` 指定が通常どおり効く
- **ダイアログの `y` / `n` キー既定バインドを廃止**（迷い入力での誤確定対策）。Enter / Esc で確定・キャンセル。`keybindings.json` で `confirm:yes` / `confirm:no` に `y` / `n` を再バインド可能。ほとんどのダイアログで Ctrl+C / Ctrl+D 2 回押しが Claude Code 終了ではなくダイアログクローズに
- `PermissionRequest` イベントは **agent フック非対応**（command / http / mcp_tool / prompt のみ）。設定してもスキップされ権限フローは変わらない。フックから allow / deny するには command / HTTP フックの decision オブジェクトを使う
- **symlink 経由の書き込みの権限判定を実際の書き込み先パスで行うように修正**。`acceptEdits` / allow ルール / auto モードがツリー外に着地する書き込みを承認しなくなる（セキュリティ修正）
- **予約マーケットプレース名を偽装する名前のプラグインマーケットプレースは追加拒否**・追加済みでもロード停止
- managed 設定が `disableFastMode` を設定している場合、非対話・VSCode・クラウドセッションでの `/fast on` は拒否される
- fullscreen の `ctrl+l` / `cmd+k` によるトランスクリプトクリア（2.1.260）をリバート（画面再描画に戻る）
- \[Claude Code on the web] 管理者の Routines 設定が Admin settings → Capabilities → Remote sessions 配下に移動

### ドキュメント改訂（v2.1.280 前後）

- **managed 設定の不正値フィールドのフェイルセーフ挙動表**が新設: `allowedMcpServers` / `allowedHttpHookUrls` / `strictKnownMarketplaces` / `availableModels` 等の allowlist 系は「修正まで空リスト強制」、`allowManagedHooksOnly` / `allowManagedMcpServersOnly` / `disableCommandPluginSources` / `disableSideloadFlags` は「`true` 扱い」
- **クラウドセッションはリポジトリ宣言プラグインをロードしない**方針に変更: `.claude/settings.json` の `enabledPlugins` / `extraKnownMarketplaces` は（単一リポジトリでも）クラウドセッションでインストールされない。claude.ai アカウントでプラグインを有効化して同期プラグインとしてロードさせる
- **サブエージェントのモデルエイリアス解決の特例**: frontmatter / per-invocation の `opus` 等のファミリーエイリアスは、メイン会話のモデルが同ファミリーの場合**メイン会話の正確なモデル（`[1m]` サフィックス含む）に解決**される。`CLAUDE_CODE_SUBAGENT_MODEL` のエイリアスは常に通常解決
- **同期スキル（`~/.claude/skills/synced/`）はダウンロード専用**: ローカル編集は claude.ai に保存されず次回同期で上書き・削除されうる。変更は claude.ai 側で行う
- **worktree 内セッションのスキル探索**: 親ディレクトリ探索は worktree ルートまで。v2.1.277 以降、worktree に `.claude/skills` がなければメインチェックアウトのプロジェクトスキルをロード
- **Invisible characters 仕様**: 貼り付け由来の不可視 Unicode 文字（tag 文字・双方向制御・ゼロ幅スペース等）を Enter 時に除去。除去があった場合は送信せず通知付きで入力欄に戻し、再度 Enter で送信（ペルシャ語・インド系文字の接合子や絵文字セレクタは保持）
- **UserPromptSubmit フックの `prompt`**: `[Pasted text #N]` プレースホルダは展開されて届く。ペーストマーキング有効セッションでは `<pasted_content id="…">` 行に挟まれる
- **SubagentStop の内部エージェント発火**: prompt suggestions や `/btw` 等の内部エージェント完了でも発火し、その場合 `agent_type` はセッション自身のエージェント名（未設定なら空文字列）。エージェントタイプを名指しする matcher は空 `agent_type` にマッチしない（省略 / `""` / `"*"` はマッチ）
- **サンドボックスの `excludedCommands` の限定効果**: 複合コマンドは全コマンドがエントリにカバーされる場合のみサンドボックス外実行（`docker *` だけでは `npm ci && docker build .` は外れない）。サンドボックス内は `$TMPDIR` がセッション temp ディレクトリに設定される
- **Claude Docs（Projects）の表**: 複数リポジトリのスレッドは各クローンが additional directory として CLAUDE.md ロード有効でアタッチされる（CLAUDE.md / skills は全リポジトリからロード、permission / hooks / env は単一リポジトリ時のみ）
...
- **サブエージェント frontmatter / `--agents` JSON に `omitClaudeMd` を追加**: `true` でユーザー・プロジェクト・ローカルの CLAUDE.md を読み込まずに起動（managed policy ファイルは読み込まれる。managed 定義のサブエージェントでは policy も読まない）。委譲プロンプトだけで完結するサブエージェント向け。`--agent` / `agent` 設定でメインセッションのエージェントとして走る場合は無視される
- **auto モード＋サンドボックスで Bash / PowerShell / Monitor にコマンド単位の `allowed_domains`**: コマンドが必要とするホストをコマンドと一緒にレビューし、そのコマンドだけに開放。それ以外のホストは拒否
- **Claude Code Remote セッション（cloud / self-hosted runner）で fast モード対応**: ホスト側の fast モード設定またはセッション内の `/fast` が組織の許可範囲で適用される
- **`claude plugin install` / `update` に `--accept-command <sha256>`**: 事前の `--json` 実行で表示されたコマンドと完全一致する場合のみ承認（`-y` の全面承認の代替）
- **`modelPricing`（managed）と Claude apps gateway `pricing` の `multiplier` が 1 超〜10 まで対応**: 社内チャージバック用のマークアップレートが可能に（従来は 1 以下の割引のみ。旧バージョンは 1 超を警告付きで無視）
- `claude self-hosted-runner --drain-marker-file <path>`: SIGTERM ドレイン時にファイルが存在すればホストドレインとしてサーバーに報告（テレメトリのみ）
- fullscreen モードの `/config` パネルがマウス対応（ホイールスクロール・クリックで値変更）

**変更**

- **Monitor の watch に常時デッドライン導入**: 既定5分・最大30分（単発 `-p` 実行では最大10分）。期限到達で watch は終了し Claude に通知が1回届くので必要なら再アームする。**無期限の `persistent` オプションは廃止**。`timeout_ms` は WebSocket watch にも適用
- **auto モードでスキル / スラッシュコマンドのインライン `!` シェルコマンドが分類器ではなく default モードの権限ルールに従う**ようになった（どのルールにも該当しないコマンドはレビュー付きツールコールとして実行）
- auto モードでサブエージェントの呼び出し元への報告が「最後のメッセージの事後レビュー」から「分類器がレビューする専用ハンドバックコール」に変更
- **dynamic workflow のサイズガイドライン変更**: Pro プランの既定が small に、medium の目安が 15 → 10 エージェント未満に縮小
- IDE の選択範囲インジケーターが `[⧉ …]` ピル表示になり、Backspace で削除して選択を送信から外せる

**セキュリティ / MCP**

- **リモートMCPサーバーの `url` / `headers` では認証情報系環境変数（`ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` / `AWS_BEARER_TOKEN_BEDROCK` / `HTTPS_PROXY` / `NPM_TOKEN` 等）が展開されず空として読まれる**: プロジェクトの `.mcp.json` やプラグインが資格情報を外部サーバーに送るのを防ぐ。`:-default` フォールバックも無視。`ANTHROPIC_BASE_URL` のようなベースURLは展開される。対象外の名前（`API_KEY` 等）は従来どおり展開。渡したい場合は独自名の変数にコピーして参照する
- Bash 権限チェックの複数の抜け穴を修正: 未知オプション後のファイル読み取り（`fmt` / `column` 等）、パターン・オプション値内のワイルドカード展開先、シェル変数宣言フラグによるコマンド偽装、二重ディレクトリ変更・サブシェル・`cd`+`git` チェーンでの `blockReadsOutsideWorkingDirectories` スキップ

**修正・改善（主要）**

- SessionStart フック実行中に送ったプロンプトは起動時（resume 含む）もフック完了まで送信待ちになり、待機中は Esc でプロンプトを入力欄に取り戻せる（フックは走り続ける）
- dynamic workflow がエージェント実行中に使用上限に達すると失敗せず一時停止し、リセット後に自動継続（条件: インタラクティブ＋claude.ai サブスク＋`autoContinueAtUsageLimit` 有効＋24時間以内リセット＋待機2回まで）
- クロスセッションメッセージ: 保留・配信・拒否・期限切れの通知が同一マシンの送信側セッションに届くようになり、`claude -p` 送信者もストリーム出力の informational `system` メッセージで受け取れる
- `/resume` / `/teleport` が前の会話のファイル読み取り追跡を引き継ぎ、未読ファイルを編集できてしまう問題を修正
- `--resume` でモデルファミリーが異なると 1M コンテキスト（`[1m]`）が落ちる問題を修正
- コンパクション後にバックグラウンドコマンド（watch タスクや dev サーバー）を二重起動する問題を修正
- アーティファクト watch の上限が 5 → 10 に拡大
- Markdown ファイルのアーティファクト公開がスタイル付きドキュメントページとしてレンダリングされるように
- 端末レンダリング性能改善（大きな diff・長いトランスクリプトの描画が高速化）
- \[VSCode] Attach Open File 設定追加（オフで開いているファイルの自動添付を停止。選択テキストは添付継続）ほか Hooks ダイアログ関連の修正多数
- \[Claude Code on the web] Routines ページが Yours / Templates タブの新レイアウトに（カレンダービュー廃止）。Cloud environments 管理画面に Custom network access オプション追加
- \[Claude Tag] スレッド中心チャンネルでのコンテキストリセット問題、PR watch の再起動後の失効、他ボット宛て指示の誤followなどを修正
- \[Code Review] 同一 findings の重複投稿、解決済みセキュリティ findings の再投稿、/ultrareview 完了セッション再オープン時の再実行などを修正

---

## v2.1.270 (2026-09-12)

- 修正: セッションが長時間走った後、Bash の読み取り専用 git コマンドが予期せず権限確認を求めるようになるリグレッション（2.1.269 で混入）を修正。このバージョンは本修正のみ
...
- 修正（**v2.1.265 の回帰のホットフィックス**）: 未文書の環境変数 `CLAUDE_CODE_USE_GATEWAY` が、v2.1.265 で単体でも Cloud gateway サインインを強制するようになり、API キー / `apiKeyHelper` / 独自認証ヘッダと併用している構成が全リクエスト `Not signed in to the Cloud gateway` で失敗していた。**`ANTHROPIC_BASE_URL` と `ANTHROPIC_AUTH_TOKEN` の両方が設定されていない限り無視される**従来挙動に戻された（設定変更は不要）
...
- **非対話セッション（`-p` + stream-json 入力、Agent SDK、クラウドセッション）で、ユーザーメッセージごとにシェルの作業ディレクトリがリセットされる問題を修正**。`cd` がターンを跨いで永続化するようになった。モノレポ等で `cd <pkg> && ...` を前提に組んでいたスクリプト系ハーネスの挙動が変わる
- **前回プロセスがツール実行中に落ちた後の再開を修正**: 最後のプロンプトが書き換えられなくなり、中断されたツール呼び出しは「中断済み」として保持されるようになった
- **`--worktree` の起動を高速化**: 大規模リポジトリで新規 worktree のチェックアウトを並列化（git 2.32+）
- 多数のファイルを読んだ長いセッションの再開時間を短縮
- 修正: Claude Code 自身の git status / diff 探査が、作業ツリー内のネストしたリポジトリが設定した clean フィルタを実行してしまう問題

**プロンプトキャッシュ（サブエージェント／エージェントチーム）**

- **修正: フォアグラウンドで起動したサブエージェントを再開すると、そのツール一覧とシステムプロンプト接頭辞が変わり、当該エージェントのプロンプトキャッシュ再利用が壊れる問題**
- **修正: エージェントチームのチームメイトと再開されたサブエージェントで、`SubagentStart` フックのコンテキストとプリロードされたスキルが、2 ターン目以降プロンプト接頭辞の外へ移動してしまいキャッシュ再利用が壊れる問題**。サブエージェントを多用するハーネスのコスト・レイテンシに直結する

**ツール出力・プラグイン**

- **ツール結果のディスク退避に 1GB の上限**を追加。会話中のプレビューには、退避ファイルが切り詰められた場合その旨が表示される
- **`--plugin-dir` に「プラグイン群を入れたフォルダ」を指定可能に**: マニフェストを持つ子フォルダがそれぞれロードされ、実行中に子フォルダを追加・削除しても拾われる
- 修正: プラグインパスにバックスラッシュを含めると macOS / Linux で symlink 封じ込めチェックを回避できる問題（セキュリティ関連）
- 修正: 名前がドット 2 個で始まるプラグインディレクトリが、プラグインルート外として誤って拒否される問題
- 修正: OS 側で状態を確認できないプラグインの既定コンポーネントフォルダ（symlink ループ等）が黙ってスキップされる問題。`/plugin` にエラーコード付きで報告されるようになった
- 修正: マーケットプレースのメタデータが `plugin.json` にしかないプラグインで、`/plugin` の Discover / Browse と `claude plugin list --json --available` に説明・表示名が出ない問題
- 変更: Installed タブと `claude plugin details` の表示メタデータは**マーケットプレースのエントリを優先**し、欠けている項目を `plugin.json` から補うようになった

**スキル・ワークフロー**

- **修正: `context: fork` のフォークスキルが、キックオフプロンプト（および `--forward-subagent-text` 指定時はテキストターン）を stream-json の progress イベントとして流していなかった問題**
- **プロンプト途中で打ったスラッシュコマンドの改善**: 単一サジェストではなく一覧表示になり（フルスクリーン外では Tab で開く）、**プラグインスキルを素の名前でも見つけられる**ようになった
- **`/workflows` のエージェント詳細を改善**: ツール呼び出しに running / failed / done のマークが付き、サブエージェントがタスクリストを持つ場合はそれを表示、Enter で入力・結果込みに展開できる
- 修正: コンテナ再起動後のワークフロー再開。実行ジャーナルが失われた再開は、全エージェントを再実行せず明確なエラーで失敗するようになった
- 修正: `claude-api` スキルのエラーコード記述（モデルアクセス失敗は 404、未対応 beta ヘッダは 400。403 ではない）

**MCP・コネクタ**

- **修正: `http` として設定した MCP サーバーがレガシーな HTTP+SSE トランスポートしか話さない場合に一切接続できない問題**。MCP 仕様どおり SSE へフォールバックするようになった
- 修正: claude.ai で接続済みのコネクタが、クラウドセッションで「要認証」と表示される問題（未対応リクエストに HTTP 401 を返すサーバー）
- 修正: リモートセッションが、コネクタ承認やサインインリンクの待機中もサンドボックスコンテナを起こしたままにする問題

**モデル・認証・組織設定**

- 修正: `/model opusplan[1m]` が `Model not found` で拒否される問題
- 修正: `/model` が設定ファイルに書けなかった場合でも「既定として保存した」と表示する問題（保存失敗と理由を表示するようになった）
- 修正: advisor ツールとその指示がリクエストごと・リクエストのモデルごとに再判定される問題。**一度だけ判定し、変わったときに会話中で通知する**ようになった
- 変更: managed 設定に `forceLoginGatewayUrl` があるマシンは、`forceLoginMethod: "gateway"` と同様に**起動時点から Claude apps gateway セッション**になる（残っている claude.ai ログインや API キーは使われない）
- 変更: Claude apps gateway セッションの OpenTelemetry は、gateway の managed 設定が `OTEL_EXPORTER_OTLP_ENDPOINT` で指定するコレクタへ**直接エクスポート**するようになった（指定がない場合は従来どおり gateway のリレー経由）
- Claude Desktop / Cowork が Claude apps gateway 経由で送るテレメトリに `user.email` / `user.groups` を追加（ターミナルセッションと同等に）
- 修正: `/login` を、managed 設定が指定する Claude apps gateway にサインイン済みのセッションで再実行すると「ゲートウェイ URL が設定されていない」と表示される問題

**UI・その他**

- **修正: 2 キーのキーボードショートカットが、2 打目が 1 秒以上遅れると（tmux 内で発生しやすい）黙ってキャンセルされる問題**。3 秒待ち、タイムアウト時は通知を出すようになった
- 修正: 権限プロンプトやメッセージ内のシンタックスハイライトで、Ruby の `?`、Erlang / Perl の `$` シジルの直後の 1 文字が欠けることがある問題
- 修正: フルスクリーントランスクリプトが、スラッシュコマンド / @ ファイル候補リストの開閉のたびに 1 行ずれる問題
- 修正: `/add-dir <subdirectory>` が、managed 設定でスキルのみをプラグイン限定にしている場合にサブディレクトリのエージェントを読み込まない問題
- 修正: Remote Control セッションが返信の最後のメッセージより先にターン終了シグナルを送る問題
- 修正: バックグラウンド（`--bg`）セッションが、アイドルタイムアウト直前にメッセージが届くとターン途中で終了されることがある問題
- 修正: VS Code / SDK セッションでトークン更新中にセッションが閉じられると再ログインを求められることがある問題
- 変更: 画像処理をランタイム組み込みの画像サポートに変更（ネイティブ画像モジュールを一時ディレクトリへ展開しなくなった）
- 変更: `.claude` フォルダ権限オプションの説明を実態（そのセッションの間、プロジェクトの `.claude` フォルダまたは `~/.claude` 内のファイルを編集できる）に合わせて修正
- 改善: Artifact ツールで他人が書いた artifact を読む際、ページを**信頼できないコンテンツとして扱い、埋め込まれた指示を中継せずフラグ**するようになった
- 改善: サイズ上限を超えた画像がデコードできない場合のエラーに、原因と対処を明記
- Windows: AppContainer / 制限トークンのサンドボックス内で Read / Write / Edit が全ファイルを拒否する問題を修正（`symlink resolution changed after permission was checked`）
- \[VSCode] 一定期間非アクティブなセッションの自動アーカイブを追加（新設定 `Archive inactive sessions`、既定 14 日）
...
- **`/permissions` に Auto mode タブ**: Auto mode 分類器ルールの閲覧・編集をダイアログから直接行えるようになった
- **Bash allow ルールの起動時警告**: サブコマンドの前にワイルドカードを置いた allow ルール（例 `Bash(git * main)`）は、サブコマンド前に挿入されたオプションにもマッチしてしまうため、起動時に警告を表示
- **`/goal` のアイドルチェックイン上限**: 長時間のバックグラウンド作業に対するアイドルセッションからの自動チェックインは、ユーザーのプロンプト間で **1 ゴールあたり最大 3 回**まで。3 回目でチェックインを一時停止する旨を通知し、次のユーザーメッセージでさらに 3 回分が許可される（v2.1.245 以前は無制限）
- **`/code-review` の自律起動を拡大**: Bedrock / Vertex AI / Foundry、Claude apps gateway 経由、テレメトリ / 非必須トラフィック無効時でも Claude 自身がレビューを開始できるようになった
- **`/cd` の設定即時反映**: 移動先ディレクトリのプロジェクト設定・hooks・`.mcp.json` サーバー（通常の承認プロンプト経由）・skills・agents が `--resume` を待たずに移動直後から有効化
- **サブエージェントの部分結果**: `maxTurns` 上限で停止したサブエージェントが、完了扱いではなく「部分的な出力」として結果を返し、`SendMessage` での継続を促すヒントを付けるようになった
- **非対話セッションの自動継続**: `-p` / SDK / クラウドセッションで、サーバーエラー・接続断・ストール により応答が途中で切れた場合、エラー終了せず自動的に継続
- **`keybindings.json` の不正アクション**: 未知のアクション名を指定したバインドがそのキーを黙って無効化していた問題を修正。該当バインドはスキップされ既定バインドが生き残り、`--debug` で警告が出る
- ターン終了時の所要時間行に完了時刻を追加（例 `✻ Sautéed for 23s · done 6:05 PM`）
- 修正: サンドボックスの「Network request outside of sandbox」権限プロンプト待機中に `Notification` フックが発火しない
- 修正: Bash 権限チェックで、末尾に `&&` / `||` がぶら下がった不正コマンドが常に承認を要求するように
- 修正: `--strict-mcp-config` セッションが、読み込むはずのない `.mcp.json` サーバーの承認を求めてバックグラウンドセッションを起動時に待たせる
- 修正: サードパーティゲートウェイ（`ANTHROPIC_BASE_URL`）向けの API キーがテレメトリ / メトリクス送信に付与されていた（認証情報は自ホストにのみ送信）
- 修正: `apiKeyHelper` が短命 JWT を返す構成で、アイドル後の初回プロンプトが API エラーになる（期限切れトークンを送信前に更新し、401/403 は静かにリトライ）
- 修正: プラグイン系の不具合多数（キャッシュの SHA 重複ディレクトリ、`<plugin>:` プレフィックスの二重表示、bare name での `claude plugin update` 失敗、BOM 付き `plugin.json`、`/reload-plugins` が `skills/*/SKILL.md` を 0 件と報告、フックエラーの `${CLAUDE_PLUGIN_ROOT}` 未解決表示）
- 修正: 動的ワークフロー中に ← / `/background` を押すと完了済みサブエージェントが再起動する（再起動対象数を提示して確認するように）
- 修正: バックグラウンドの保持期間スイープが、自分で作った `.claude/worktrees/` 配下の git worktree を削除する
- 修正: MCP ツール呼び出しの割り込みが「出力なしで完了」と報告される / スキーマが空 `{}` のパラメータが JSON 文字列として送信される
- 修正: 巨大ファイル上書き後に Write ツールが "Out of memory" を報告 / フリーズする
- 修正: プラン モードで終了したセッションが VS Code 拡張・`claude -p --continue`/`--resume` でプランモード外で再開する
- 改善: フルスクリーン / Ctrl+O トランスクリプト表示のメモリ使用量（各行がトランスクリプト全体のツールルックアップを保持しない）
...
- バグ修正・安定性改善のみ（公式 changelog に個別項目の記載なし）
...
- **組み込み出力スタイル「Concise」追加**: 前置きやナレーションを省いて結果から書き出す出力スタイル。作業の徹底度は変えずに応答だけを簡潔にする。`/config` の Output style から選択（`outputStyle` 設定キーでも指定可）
- 修正: LLM ゲートウェイまたはカスタム base URL を使うセッションでプロンプトキャッシュが効かない問題

---

## v2.1.236 (2026-08-19)

- **`ANTHROPIC_DEFAULT_MODEL` 環境変数追加**: 新規セッションが開始するモデルの既定値を指定する。`--model` フラグ / `ANTHROPIC_MODEL` / 設定ファイルの `model`（`/model` で保存した選択を含む） / 組織既定モデルのいずれも指定がない場合のみ適用される。`/model` で保存した選択は次回以降も本変数より優先される（`ANTHROPIC_MODEL` は逆に毎回変数側へ戻る）点が違い。`/model` ピッカーの Default 行には「Set by ANTHROPIC_DEFAULT_MODEL」ラベルが表示される。`default` / `inherit` / `opusplan` / `haiku` を指定した場合、`enforceAvailableModels` 有効時、`availableModels` や組織のモデル制限で除外される場合、アカウントで利用できないモデルの場合は無視される。`--resume` / `--continue` / `/resume` での再開時も（本変数が効く条件下では）トランスクリプト保存モデルではなく本変数のモデルで開始
- **`SendMessage` に `notify_when_idle` 追加**: 同一マシン上の他 Claude Code セッションに対し「次にアイドルになったとき一度だけ通知して」と依頼できるオプトイン・ワンショットの通知（ポーリング不要）。macOS / Linux 対応
- **サンドボックス（macOS）のワイルドカード read-deny 強化**: `**/.env` のようなワイルドカード read-deny ルールが許可済み read 領域の内部でも優先されるようになり、マッチしたディレクトリの配下も対象に含まれ、拒否対象ファイルのリネームによる回避もできなくなった
- **auto モードでの `Monitor` 権限の扱い**: auto モード実行中は `Monitor` の allow ルールを一時的に無効化し、`Monitor` コマンドを Bash コマンドと同じ基準で審査するように
- **auto モードの分類器を Bedrock / Vertex AI / Foundry・テレメトリ無効時にも改善**: Claude API と同じ既定（重大度スコア付き分類を含む）を使用。また `status.showUntrackedFiles=no` を設定したリポジトリで git 状態チェックが「クリーン」と誤判定されないように
- **スラッシュコマンドのタイポ時に曖昧マッチしない**: 存在しないコマンドやこのセッションで使えないコマンドで Enter を押すと、最も近いコマンドを勝手に実行せずその旨を報告する（プレフィックスとエイリアスは従来通り実行）
- **`SendMessage` のバースト事前拒否**: 短時間の連投が受信側インボックスの受容量を超える場合、以降のメッセージを「送信済み」と報告してから破棄するのではなく、送信前に拒否する
- **`/goal` の自動チェックイン**: 長時間のバックグラウンド作業の背後でゴールが停滞しているアイドルセッションが、ユーザーの復帰を待たず 30 分後（以降 1h・2h）に自動でチェックインする
- **Remote Control のオフライン検出高速化**: CLI 終了や端末クローズを数秒でオフラインとして反映
- **`/usage`**: Team / Enterprise メンバーにも usage-credits の消費行を表示（未消費時は 0% の行を表示）
- **print / SDK モードの SIGTERM**: 終了前に中断ターンや合成ツール拒否を記録しなくなった（実行中コマンドの終了と終了コード 143 は従来通り）
- **起動パフォーマンス改善**: セッションカウンタの書き込みをバックグラウンド化
- **\[VSCode] スクリーンリーダー対応**: トランスクリプトの応答・権限リクエスト・エラー・状態変化のライブ読み上げと、ターン単位の見出しナビゲーションを追加
- 主な修正: セッションが切り替えた先のディレクトリが削除された後にクリップボードコピー / バックグラウンド処理 / バックグラウンドセッション / ローカル MCP ログが壊れる問題（2.1.229 以降）、フルスクリーンレンダラが一度起動に失敗すると以後毎回終了する問題（クラシックレンダラにフォールバック）、`/model` ピッカーが端末より高く描画される問題、閉じタグ不正でメッセージ本文が summary に入ると `SendMessage` が拒否される問題、WSL で Windows interop 無効時の `powershell.exe` などサブプロセス起動失敗による未処理 promise rejection（2.1.234 のリグレッション）、フルスクリーンモードの再描画・空白帯の問題、managed 設定の承認プロンプトが表示されないまま最初のキー入力を承認として拾う問題、tmux（iTerm 統合）でターミナルタブタイトルが 960ms ごとにアニメーションする問題、`~/.claude.json` のゲストパス報酬が壊れているとスピナー Tips が出なくなる問題、作業ディレクトリ削除後のセッションでスキル hot-reload がエラーになる問題（2.1.229 以降）、セルフホストランナーのセッションが post-session フック完了前に別ランナーで再開される問題、セッションリキャップ（`/recap` 含む）の暴走（400 文字・語境界で打ち切り）

---

## v2.1.235 (2026-08-18)

- **`spellcheck` 設定追加**: プロンプト入力欄の誤字を入力中に下線表示する任意設定。外部スペルチェッカー（`aspell` / `hunspell` / `ispell` の順に PATH から検出）を利用し、既定は無効。ユーザー設定 / `--settings` / Managed 設定からのみ読み込まれ、プロジェクトの `.claude/settings.json` は無視される
- **クラウドセッションのメモリ/CPU 改善**: `/ultrareview` や `/autofix-pr` などがバックグラウンド実行中、イベントストリームを更新のたびに再走査・再描画しなくなった
- **権限ダイアログの整合性向上**: 表示テキストと「二度と確認しない」の対象範囲が実際の付与内容と常に一致するようになり、内容を完全表示できない場合は「二度と確認しない」を提示しなくなった
- **組み込み `grep` の改善**（ネイティブ macOS/Linux ビルド）: 病的なパターンがメモリを食い潰さず即座に失敗するようになり、`-m N` と `-A/-C` の併用で正しいコンテキストを出力
- **コンテキスト上限エラーの改善**: auto-compact が無効な場合はその旨と `/config` からの再有効化を案内
- **`SendMessage` の事前拒否**: セッション間配送に対して大きすぎるメッセージを、黙って破棄せず送信前に拒否
- **Remote Control**: `claude rc` がインタラクティブ起動と同じエンタープライズゲートウェイ可用性チェックを適用
- Vim モード: 詳細トランスクリプト（ctrl+o）のトグルやパネルを閉じた際に NORMAL モードとカーソル位置を保持
- 主な修正: 言語サーバーがセッション中に切断/再接続するとプロンプトキャッシュ全体が無効化される問題、深さ3以上のネストしたマークダウンリストのずれ（折り返し行のぶら下げインデントも追加）、複数行プロンプトでスラッシュコマンド/キーワード/メンションのハイライトが数文字ずれる問題、権限プロンプトのコメント欄での Shift+Tab が編集を承認しセッション全体の編集権限を付与してしまう問題、general-purpose エージェントが使えないセッションで Agent ツールが既定として宣伝していた問題（`subagent_type` 省略時に利用可能エージェント一覧付きのエラーを返すよう変更）、ノートブックやセルを読めない場合に承認ダイアログが既存セル内容を黙って省略していた問題、Claude 応答中に実行したスラッシュコマンドが HTML エンティティで表示される問題、バックグラウンド自動更新後にフッターへ再起動通知が出ない問題、未完了タスクが残るセッションの再開時に展開済みタスク一覧（`ctrl+t`）が常に折りたたみ状態で始まる問題、ダイアログで矢印キーと Enter を素早く連続入力すると直前のハイライト項目が選ばれる問題、\[VSCode] 複数の Claude パネルを持つウィンドウの復元/リロード時にタブ間でフォーカスが勝手に移動する問題

## v2.1.234 (2026-08-17)

- **`teammateDefaultModel` 削除**: 設定キーが削除され、残存値は無視される。チームメイトのモデルはプロンプトで明示するか `CLAUDE_CODE_SUBAGENT_MODEL` で指定する
- **`SessionEnd` の `bypass_permissions_disabled` 廃止**: Claude Code は当該 reason を送出しなくなった。既存フックの matcher から削除が必要
- **`CLAUDE_CODE_GOAL_CHECKIN_MINUTES` 追加**: `/goal` 実行中にバックグラウンド作業がゴール評価を待たせ続けたとき、Claude に状況確認を促す間隔（デフォルト 30 分、`0` で無効、最大 10080 分）
- **`CLAUDE_CODE_PROJECT_DIR_NAME` 追加**: セッションごとに独自 config ディレクトリを持つホストが、プロジェクト単位トランスクリプトディレクトリに短い名前を指定できる
- **`selection:clear` キーバインドアクション追加**: アプリ内テキスト選択の解除をキーに割り当て可能（既定は未バインド）。agents ビューでも動作
- **フッター/ステータスラインの GitLab MR バッジ**: GitLab リモート＋認証済み `glab` CLI のリポジトリで、オープンな MR を `MR !N` として表示（draft / pending / green の状態別表示）
- **使用量上限リセット時の自動継続**: claude.ai の使用量上限がリセットされるとセッションを自動再開。`/config` の「Continue automatically at usage limit」で無効化可能
- **アカウントメールの取り扱い明確化**: Claude はアカウントメールを本人識別にのみ使用し、ユーザーの指示なく無関係なサービスに送信しないよう指示される
- **セキュリティ（NTLM 資格情報漏洩対策の仕上げ）**: リモートファイル読み込み・セッション復元・CLAUDE.md インポート・ワークフロースクリプト・ファイルアップロードが Windows NT 名前空間（`\??\`）パスを拒否するようになり、事前承認前のファイルアクセス経路を全面的に塞いだ
- 主な修正: 長時間セッションでコンパクション後に auto モードがサンドボックスコマンドのネットワークアクセスを再確認・拒否し続ける問題、バックグラウンドサブエージェントの権限プロンプト応答時にセッションスコープの回答（拒否含む）が破棄される問題、非ストリーミングフォールバック経路（サードパーティゲートウェイ経由が典型）で thinking / text フィールド欠落のブロックを受け取るとクラッシュする問題、`ListAgents` からコピーした 200 文字上限・絵文字多用のセッション名を `SendMessage` が拒否する問題、MCP 診断が解決済みシークレットを表示していた問題（スコープ競合警告は `${VAR}` 形式、接続失敗詳細はサーバーオリジンのみ表示）、`strictKnownMarketplaces` が git が実際に接続するホストと異なる SCP 形式の git マーケットプレースソースを受理する問題
