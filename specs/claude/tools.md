...
| `TaskOutput` | No | **v2.1.277 で削除**。バックグラウンドタスクの出力はタスクの出力ファイルパスを `Read` で読む（`taskOutputMaxChars` / `TASK_MAX_OUTPUT_LENGTH` も無効化） |
...
- `cd` はプロジェクトディレクトリまたは追加作業ディレクトリの内側にいる限り持ち越される。**この持ち越しはユーザーの後続メッセージに対して実行するコマンドにも及ぶ**（ターンを跨いで永続。公式ドキュメント明文化）。**サブエージェントのセッションは作業ディレクトリ変更を持ち越さない**。外に出ると自動リセットされ結果に `Shell cwd was reset to <dir>` が付く。`CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR=1` で持ち越し自体を無効化
...
- **`Read` の deny ルールにマッチするパスは Edit / Write も拒否される**（そこへの新規ファイル作成も含む）。Read / Edit の deny ルールは Claude Code が認識する Bash のファイルコマンド（`cat`, `head`, `tail`, `sed` 等）にも適用される。**2026-09-06 時点のリファレンスで `> file` / `< file` 等 Bash のリダイレクト先にも適用されることが明記された**。任意のサブプロセスには適用されないため、OS レベルで強制するにはサンドボックスを有効化する
...
- `pattern` / `path` に NUL バイトが含まれる場合はエラーを返し、Claude に除去を求める
