...
- **再接続・リトライの整理（ドキュメント改訂）**:
  - **セッション中に切断したリモートサーバー**: 指数バックオフで最大 5 回再接続（初回 1 秒、以降倍増）。対話セッションでは `/mcp` に pending 表示、5 回失敗で failed（再認証が必要な場合は「要認証」）となり `/mcp` から手動再試行できる。failed になった時点で `MCP server "<name>" disconnected · open /mcp to reconnect` の通知が表示される（2026-09-18 ドキュメント改訂で明文化）。`claude -p` / Agent SDK でも同じスケジュールで再接続するが `/mcp` パネルは無い
  - **403 insufficient_scope 時のフロー（2026-09-18 ドキュメント改訂）**: ツール呼び出しがサーバーから 403 `insufficient_scope` を受けると、その呼び出しは要求スコープ名を含む `needs additional permissions` メッセージで失敗し、`/mcp` では要認証表示になる。**Claude Code は再認証時にサーバーが名指ししたスコープではなく pinned な `oauth.scopes` を要求する**ため、不足スコープを `oauth.scopes` に追加してから `/mcp` で再認証しないと、取得し直したトークンにも不足スコープは含まれない
...
Claude API はリクエスト中の全ツールの入力スキーマを検査し、1 つでも不正だとリクエスト全体を 400 で拒否する。そのため Claude Code はサーバーのツール読み込み時に API の検査のうち 2 つを自前で実行し、**当該除外を有効化する機能フラグを受け取っているデプロイでは**、失格するツールだけを除外して他のツールを生かす。
...
この除外は Anthropic から取得する**機能フラグ**で有効化される。[機能フラグ取得がオフのデプロイ](https://code.claude.com/docs/en/env-vars#features-that-need-feature-flag-fetching)や、フラグが一度も届いていないマシン（エアギャップ環境等）では、検査自体は行ってログに残すが**ツールはそのまま送信される**ため、含まれるリクエストが「ツールを位置で名指しする 400 エラー」で失敗する。v2.1.216 より前はどのデプロイでも検査していなかった。

なおルートレベル `anyOf` / `oneOf` / `allOf` の書き換え（v2.1.195 以降。それ以前は該当ツールを一律スキップ）はこれとは別系統の扱いで、機能フラグ取得がオフでもフラグが未着でも従来どおりの挙動を保つ。
...
- **クライアントシークレットはサーバー追加時にのみ設定可能**（2026-09-13 リファレンス改訂で明文化）: `claude mcp login` や `/mcp` からの認証時は保存済みシークレットが使われ、プロンプトも `MCP_CLIENT_SECRET` の読み取りも行われない。後からシークレットを追加・変更するには `claude mcp remove <name>` してから `--client-secret` と同じ `--scope` で再追加する
- **エラーメッセージの `${VAR}` 展開抑止（v2.1.268）**: `/mcp`・`claude mcp list`/`get`・ログインエラーが `${VAR}` から解決したシークレットを表示しないようになった。接続失敗メッセージに付くオリジンも、local / project / user / managed スコープでは設定に書かれたままのホスト表記（`${VAR}` 未展開）で表示される
