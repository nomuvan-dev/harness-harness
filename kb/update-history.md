...
Claude Code に **v2.1.247**（2026-08-26）、Codex CLI に **0.150.0**（2026-08-26）/ **0.150.1**（2026-08-27）がリリースされた。公式ドキュメント 35 ページのハッシュ比較で 12 ページの変更を検出し、うち 9 ページ分を specs に反映。スキルエコシステム巡回は前回（2026-08-25）から 7 日以内のためスキップ。

**1) `SendFeedback` ツール / Claude 起草フィードバック（v2.1.247、ツール自体は v2.1.238+）**

Claude が Claude Code へのフィードバックレポートを起草し、**ローカルの `~/.claude/feedback/drafts/`** にキューする。ツールが呼ばれた時点では何も送信されず、ユーザーが `/feedback` のキューでレビューして送信を選ぶまで Anthropic には届かない。起草の契機は「ツールが繰り返し失敗した」「依頼に対応できなかった」「ミスを指摘された／自分で気づいた」「ユーザーがフィードバック提出を求めた」。

ハーネス設計上の含意は 2 つ。ひとつは**カードによる会話フローへの割り込み**で、既定 `notify` は 1 セッション最大 3 枚のカードをプロンプト上部に出す。自動化・無人ワーカーでは `feedbackDrafts: "quiet"` か `"off"` を選ぶのが素直。もうひとつは**トランスクリプト送信のトグル**で、キューからの送信では既定 `yes`（会話全体が Anthropic へ行く）、カードからの送信では常に含まれない。機密プロジェクトでは `feedbackDrafts: "off"` を managed settings で固定するのが安全側。

`feedbackDrafts` は user / managed のみ有効で、**project / local 設定からは読まれない**点に注意（プロジェクト側でリポジトリに置いても効かない）。非対話 `-p` / Agent SDK / クラウドセッション / Bedrock・AWS・Vertex・Foundry / ZDR 組織では、そもそもツールが提供されない。

**2) `spinnerTipsOverride` の構造化（v2.1.247）**

`tips` にオブジェクト形式（`id` / `text` / `cooldownSessions` / `priority`）と、外部 JSON を指す `tipsFile`、接頭辞 `label` が加わった。組織独自の Tips を組み込み Tips と同じローテーションに混ぜられる。スコープの扱いが独特で、**project / local 設定からはプレーン文字列の tips しか読まれない**（オブジェクト・`tipsFile`・`label`・`excludeDefault` は user / `--settings` / managed のみ）。チーム共通のリマインダをリポジトリにコミットする使い方だと、文字列形式に落とすか managed 配布に切り替える判断が要る。

**3) セッション間メッセージの表示がプレビュー化（v2.1.247）**

到着メッセージが会話に全文表示されなくなり、`› Message from @api-worker: ... (ctrl+o to expand)` の 1 行プレビューになった。**Claude 側は従来どおり全文を読む**ので挙動は変わらないが、人間がログを追う運用（マルチセッション協調のデバッグ等）では `Ctrl+O` か `--verbose` が必要になる。あわせて、外部からソケットへ投函する場合は **30 秒以内に完全な 1 行を送らないと切断される**ことが明記された。遅いコマンドの出力を投げるなら「先に取得してから接続」が正しい順序。

**4) キーバインドの追加と非 Latin レイアウト修正（v2.1.247）**

`chat:queueSubmit`（既定 `Ctrl+X Enter`）は「割り込まずキューに積む送信」で、Claude 作業中の追加指示をターン中断なしに渡せる。リスト操作に `select:pageUp` / `pageDown` / `first` / `last` が追加されたが、実際に効くのは `/skills` メニューのみ。また、Kitty キーボードプロトコル対応端末（Ghostty / Kitty / WezTerm / iTerm2）でキリル等の非 Latin レイアウト使用時に Ctrl ショートカットが発火しなかった問題が修正され、US 配列の物理位置でマッチするようになった。

**5) MCP のステータス表示と警告の整理（v2.1.238〜v2.1.247）**

`claude mcp list` / `get` の「接続を試みずに出る設定ステータス」が 3 種（`⏸ Pending approval` / `✘ Rejected` / `⊘ Disabled for this project`）に整理された。v2.1.238 より前は無効化済みサーバーにもヘルスチェック接続していた。設定警告に「複数スコープでの同名サーバー定義」（OAuth サインインがエンドポイント単位で保存されるため、プロジェクトごとに再サインインが要る）と「未設定の `${VAR}` 参照」（警告した上で未展開のままロードされる）が加わった。

`--strict-mcp-config` の挙動も明確になった。**v2.1.246 以降はロードしないプロジェクトスコープサーバーの承認待ちをスキップする**。それ以前は strict セッションでも承認待ちになり、バックグラウンドセッションが起動時に固まっていた。無人セッションのハーネスでは v2.1.246 を下限に置くのが安全。

**6) プラグインスキル名の接頭辞二重付与（v2.1.246 で修正）**

frontmatter の `name` が既にプラグイン接頭辞を含む場合（`name: my-plugin:fancy`）、v2.1.216〜v2.1.245 は `/my-plugin:my-plugin:fancy` のように二重化していた。v2.1.246 以降は重ねない。自作プラグインで接頭辞込みの `name` を書いている場合、対応バージョン幅によって表示が変わる。

**7) Codex `Interrupt` フック（0.150.0）**

アクティブなトップレベルターンが中断されたときに command / MCP ハンドラを実行できる新イベント。Codex のフックイベントは 0.133.0 で 6 → 0.148.0 で 11 → 0.150.0 で 12 に増えた。**公式 hooks ドキュメントには 2026-08-28 時点で未記載**で、根拠はリリースノート（#40511）のみ。中断時のクリーンアップ（ロック解放、一時ファイル削除、ステータス更新）をハーネスに組み込む余地ができた。あわせて、機能フラグの現行キーが `features.hooks` で `features.codex_hooks` は非推奨エイリアスであることを反映。

**8) Codex 0.150.0 のその他**

タスクの `@` メンション（他タスクの読み取り・作成・メッセージ送信をエージェントに依頼できる）、無名タスクの自動命名と `/rename` のタイトル提案、`/copy` のピッカー、`tui.keymap` による権限モードサイクルのバインド。**未信頼プロジェクトではプロジェクトレベルの `AGENTS.md` 指示が供給されなくなった**のはハーネス側の前提に関わる変更で、信頼していないリポジトリでは AGENTS.md が効かない。

### 更新ファイル

- `specs/claude/changelog.md` — v2.1.247 追加
- `specs/claude/tools.md` — `SendFeedback` をツール一覧に追加、§6 として詳細節を新設
- `specs/claude/configuration.md` — `feedbackDrafts`、`spinnerTipsOverride` の構造化、`CLAUDE_CODE_SEND_FEEDBACK`、機能フラグ note
- `specs/claude/skills-and-commands.md` — `/skills` の操作拡張、`/feedback` のドラフトキュー、`/keybindings` の追加バインドと非 Latin レイアウト、プラグインスキル `name` の接頭辞規則
- `specs/claude/mcp.md` — 設定ステータス 3 種、警告 4 種、認証失敗時のオリジン表示、`--strict-mcp-config`、Anthropic ホスト型コネクタ
- `specs/claude/agent-teams.md` — 到着メッセージのプレビュー表示、`/rename` の共有記録、ソケット投函の 30 秒制約
- `specs/codex/changelog.md` — 0.150.0 / 0.150.1 追加
- `specs/codex/configuration.md` — `Interrupt` フック（12 イベント）、`features.hooks` へのキー名更新、`tui.keymap` 節を新設
...
### 検出・更新
- **Claude Code**: v2.1.228（2026-08-11）を検出 → `specs/claude/changelog.md` に追加。主にバグ修正だが (1) Write ツールが新モデルで未読ファイルを上書き可能に（Edit と同ルール）、(2) claude.ai 同期スキルのハードニング、(3) クロスセッションメッセージのインライン表示 が収載価値あり → 新規 API/設定項目はなく configuration/hooks/skills/mcp の specs 更新は不要
- **Codex**: 安定版 0.147.0（2026-08-07）据え置き。新規は rust-v0.148.0-alpha.7〜9（プレリリースにつき収載対象外）→ 更新不要
- **スキルエコシステム巡回**: 前回 2026-08-11 実施済み（7 日以内）のため Phase 3.5 スキップ

### 更新ファイル
- `specs/claude/changelog.md`（v2.1.228 追加）

## 2026-08-12 — 公式ドキュメント巡回

### 検出・更新
- **Claude Code**: v2.1.227（2026-08-10）を検出 → `specs/claude/changelog.md` に追加。内容はスラッシュコマンドメニュー改善・バグ修正・パフォーマンス改善のみで新機能なし → configuration/hooks/skills/mcp 等の specs 更新は不要
- **Codex**: 安定版 0.147.0（2026-08-07）据え置き。新規は rust-v0.148.0-alpha.7（2026-08-11）のみ（プレリリースにつき収載対象外）→ 更新不要
- **スキルエコシステム巡回**: 前回 2026-08-11 実施済み（7 日以内）のため Phase 3.5 スキップ

### 更新ファイル
- `specs/claude/changelog.md`（v2.1.227 追加）
